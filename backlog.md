...
| UI-40 | Task | P1 | Ready | M | gateway-web | Diff legal units, not English prose sentences. | `apps/gateway-web/lib/text-diff.ts`: `alignNodes` remains the first axis. `diffText` / `splitSentences` using `(?<=[.!?])` run **only** on a concatenated leaf body that has no child nodes. When both sides have `children`, diff by node (`kind` + `label`/`number`), then word-diff leaf `body`. Unit tests: (1) two trees that differ by one `sentence` node do not split on `Abs.`/`Art.`; (2) a single unsplit paragraph still word-diffs. `text-diff.test.ts` covers both. Finding [Q-7](#q-7-english-sentence-splitter-on-german-law). Sprint 26. | UI-10, ARCH-5 |
| UI-41 | Task | P1 | Ready | S | gateway-web | Axe color-contrast is on. | In `e2e/a11y.spec.ts`, remove `{ rules: { 'color-contrast': { enabled: false } } }`. Fix any violations on the routes already visited (home, search, login, DE version, article, compare, editor after sign-in). A single route may be skipped only with a comment pointing at a follow-up ID; no global disable. Finding [Q-12](#q-12-axe-contrast-disabled). Sprint 26. | UI-7, UI-14 |
...
| ED-5 | Task | P1 | Ready | S | editor-service | Session status is an enum, not a string. | Kotlin `enum class EditSessionStatus { OPEN, REVIEWING, APPROVED, PUBLISHED }` (DB values may stay lowercase with a converter). Flyway CHECK on `edit_sessions.status`. `requireStatus` compares enums. Illegal transition still 409. `EditorApiTest` unchanged in HTTP behaviour. Finding [Q-10](#q-10-stringly-typed-workflow). Sprint 24. | ED-1 |
...
| SEC-9 | Task | P2 | Ready | S | all services | Allow-list `X-Correlation-Id`. | Accept `^[A-Za-z0-9._-]{1,64}$` or mint a UUID. Prefer landing this in PLAT-5 instead of editing eight copies. Finding S-B-15. Sprint 26. | PLAT-5 |
...
| <a id="q-3-amendment-service-is-a-seed-dump"></a>Q-3 | Amendment-service is a seed dump: `AmendmentController` → repository only; change rows are SQL fixtures; real LCS lives in `gateway-web/lib/text-diff.ts`. | Heavy | 25 | AMD-5 |
| <a id="q-4-dual-write-articles--content_nodes"></a>Q-4 | Dual write: `ArticleRepository.replaceForVersion` INSERT/DELETE both `articles` and `content_nodes`. Tree-is-source-of-truth is a migration comment. | Heavy | 24 | CNT-5 |
| <a id="q-5-unstable-article-identity"></a>Q-5 | `replaceForVersion` always `UUID.randomUUID()`. Amendment `node_id` and compare-by-id break across re-import / copy. | Heavy | 24 | CNT-6 |
| <a id="q-6-eight-cloned-microservices"></a>Q-6 | Eight copies of `CorrelationIdFilter`, Dockerfile, logback, `NotFoundException`, SmokeTest. House style “copy a sibling.” amendment/audit do not earn a process until they have domain logic / auth. | Heavy | 26 (fold later) | PLAT-5, GOV-7 |
| <a id="q-7-english-sentence-splitter-on-german-law"></a>Q-7 | `text-diff.ts` splits on `[.!?]`. GG units are Art. / Abs. / Satz. | Margin | 26 | UI-40 |
...
| <a id="q-9-search-uses-simple-tsvector"></a>Q-9 | `to_tsvector('simple', …)` — no German stemming, weak article-number search. | Margin | 26 | SRC-8 |
| <a id="q-10-stringly-typed-workflow"></a>Q-10 | Edit session status is `'open'` / `'reviewing'` / … strings. | Margin | 24 | ED-5 |
| <a id="q-11-phantom-citation-service"></a>Q-11 | `amending_law_citation_id` is an opaque UUID to a service that does not exist (SRV-5). | Margin | 25 | AMD-6 |
| <a id="q-12-axe-contrast-disabled"></a>Q-12 | `e2e/a11y.spec.ts` disables `color-contrast` globally. | Margin | 26 | UI-41 |
...
| S-B-15 | `X-Correlation-Id` echoed without an allow-list (eight copies). | Low | Open | 26 | SEC-9, PLAT-5 |
...
### Sprint 24: One content write model
**CNT-5**, **CNT-6**, **ED-5**. Findings [Q-4](#q-4-dual-write-articles--content_nodes), [Q-5](#q-5-unstable-article-identity), [Q-10](#q-10-stringly-typed-workflow).

Stop dual-writing `articles`. Optional client node ids; `predecessor_id` on copy. Edit session status is an enum + CHECK constraint.

Owner: content-service + a small editor follow-up (predecessor on copy, status enum).

### Sprint 25: Amendment-service earns its process
**AMD-5**, **AMD-6**. Findings [Q-3](#q-3-amendment-service-is-a-seed-dump), [Q-11](#q-11-phantom-citation-service).
...
### Sprint 26: Shared platform, German search, honest diffs
**PLAT-5**, **SEC-9**, **SRC-8**, **UI-40**, **UI-41**. Findings [Q-6](#q-6-eight-cloned-microservices), [Q-7](#q-7-english-sentence-splitter-on-german-law), [Q-9](#q-9-search-uses-simple-tsvector), [Q-12](#q-12-axe-contrast-disabled), S-B-15.

One Gradle `platform` module; delete eight `CorrelationIdFilter` / `NotFoundException` copies. The shared filter allow-lists `X-Correlation-Id` (SEC-9). Search `tsv` uses `german` for `de`. Compare diffs outline nodes before `[.!?]`. Axe color-contrast enabled.

Owners: repo/gradle, search-service, gateway-web. Not one PR — platform first, then search, then UI.
