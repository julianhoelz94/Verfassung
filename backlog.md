...
### Sprint 34: Two-axis ADR
Ready: **ARCH-8**.

Write ADR 0005. Two axes (legal versions × editorial revisions). Catalog hop kinds `initial` / `legal` / `editorial_correction` only. Change records are comment + documents, not an errata enum. Pins, `needs_review`, and the at-most-one forward/backward association per law snapshot. Link the canvas **and** [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) § Sprint UI reference from the ADR.

Owner: docs. No L. Do not start CAT-6 until this is Done.

### Sprint 35: Catalog legal vs editorial chains
Ready: **CAT-6**.
...
Sprints 29–33 are closed. Next is Sprint 34 (**ARCH-8**). Do not start **SRV-7 (MCP)** until asked.
