...
- **Snapshot chain (ARCH-7):** each constitution has exactly one linear published-snapshot chain (no branches). A hop is `initial`, `legal_amendment`, `official_errata`, or `editorial_correction`. ARCH-6 still forbids PATCH of a published tree.
- **Amendments (ARCH-7):** an amendment is a curated legal event (which law changed the text, and how). It is not auto-created from an editorial publish. Change rows can target a whole article or any node. Version ids on an amendment are optional (historical laws may be recorded without storing every old snapshot).
- **Covert histories:** (1) editorial snapshot hops are omitted from public version lists and the public timeline; (2) each amendment has its own linear revision chain, staff-only; the public sees the latest published revision.
...
Sprints 29–33 are closed. Do not start **SRV-7 (MCP)** until asked.

## Open Product Questions
- **Decided (ARCH-6 + ARCH-7):** published constitutional versions are immutable snapshots (no in-place PATCH). Public amendment timeline = curated **laws** (and official gazette errata), not editorial version control. Snapshot chain and amendment revisions are both **linear** (no branches). Writer typos stay in an open edit session. A transcription error missed after publish is an `editorial_correction` hop: public latest text updates, public timeline does not. Assumptions baked into Sprints 29–33 (challenge these if wrong): (a) a law may be recorded without source/target snapshot ids; (b) amendment metadata skips the reviewer queue (editor drafts, publisher publishes); (c) legal/errata article-text publish must attach an `amendmentId`; (d) anonymous `GET` of an editorial snapshot URL is 200 if you know the id, but the hop is omitted from public lists; (e) withdraw hides a law from the public, it is not hard-deleted.
