...
| ING-4 | Story | P1 | Ideas | L | amendment-service | Amendments as change sets → new version. | Import/editorial path that records a change set and materializes a new content snapshot. Executable slice is AMD-5 (compute + persist) plus ED-4 (immutable successor). Keep this ID until an ingestion-specific change-set upload exists. | DB-5, AMD-5, ED-4 |
...
| <a id="q-8-swallowed-reindex--no-outbox"></a>Q-8 | Publish reindex is best-effort HTTP inside `@Transactional`; failures logged and swallowed. No outbox (ADR 0001 deferred it). | Margin | 27 | SRV-4, PLAT-6 |
...
| <a id="q-13-e2e-against-a-mock-api"></a>Q-13 | Playwright points `*_API_URL` at `e2e/mock-api.mjs`. Proves the UI, not catalog/content agreement. | Margin | 27 | CI-6 |
...
Recommended order after Sprint 15: **22 → 23 → 19 → 20 → 21 → 16 → 17 → 18 → 28 → 24 → 25 → 26 → 27**.
...
### Sprint 27: Outbox and a real publish journey
**SRV-4**, **PLAT-6**, **CI-6**. Findings [Q-8](#q-8-swallowed-reindex--no-outbox), [Q-13](#q-13-e2e-against-a-mock-api).

Document `version.published` / `amendment.recorded` / `search.reindex-requested` (no broker). Editor outbox retries reindex; preview shows `searchIndexStatus`. CI boots catalog+content(+identity/editor) and asserts the source version is unchanged after publish. Playwright mock API stays for UI e2e.
...
- **Decided (ARCH-6):** published constitutional versions are immutable snapshots. Official errata, if ever needed, are a new version — not an in-place PATCH. --- Shouldn't there be a difference in handling official errata and changes to inofficial typos made on the page. How do you handle typos made by a writer and how do you handle typos overlooked by the reviewer?
