...
| SEC-1 | Story | P0 | Ready | L | gateway-web | Upgrade Next.js off the EOL 14.2.5 line. | Move `next` and `eslint-config-next` to a supported 15.5.x (or current LTS) release that includes GHSA-89xv-2m56-2m9x. Keep React 18 if 15 still supports it. Re-run `npm test`, `lint`, `build`, and Playwright; refresh visual snapshots if the App Router markup shifts. Finding [S-F-1](docs/security-review-2026-09-08.md). Sprint 28. | UI-ED-1 |
| SEC-2 | Task | P1 | Ready | M | infra/caddy + gateway-web | Content-Security-Policy with nonces. | Caddy (or `next.config.js` `headers()`) ships a CSP including `frame-ancestors 'none'`. Next 14/15 inline scripts need nonces or `'unsafe-inline'` documented as temporary. Exempt `/api/docs/*` if Swagger stays. Finding S-F-5. Sprint 28. | SEC-1 |
| SEC-3 | Task | P1 | Ready | S | infra/caddy | Restrict OpenAPI/Swagger in non-local profiles. | `/api/docs/*` is not reachable on production (Caddy env import, `basicauth`, or `forward_auth` to identity `/me`). Local-stack may keep it. Finding S-F-15. Sprint 28. | OPS-9 |
| SEC-4 | Story | P0 | Ready | M | audit-service | Authenticate audit event reads. | `GET /events` returns 401 without a Bearer that identity `/me` accepts, and 403 unless `admin` or a new `audit:read` scope. MockMvc covers 401 and authorized 200. gateway-web does not call this yet. Finding S-B-1. Sprint 28. | GOV-6 |
| SEC-5 | Story | P0 | Ready | M | catalog-service + identity-service | Catalog publish is publisher-only. | `POST /versions/{id}/publish` requires `publisher`/`admin` or a `catalog:publish` scope (add to `ServiceTokenService.ALLOWED_SCOPES` and seed/editor tokens). Editors get 403. Finding S-B-3. Sprint 28. | GOV-6, IDN-8, IDN-14 |
| SEC-6 | Task | P1 | Ready | M | all services | HTTP client timeouts. | Every `RestClient` uses connect ≤2s and read ≤5s (`spring.http.client.*` or a shared factory). Identity’s audit call must not stall login unbounded. Finding S-B-7. Sprint 28. | PLAT-5 |
| SEC-7 | Task | P1 | Ready | M | identity-service | Service token expiry and rotation. | New Flyway `expires_at`; lookup rejects expired rows; admin UI/API can rotate. Finding S-B-16. Sprint 28. | IDN-14 |
...
| SEC-10 | Task | P2 | Ready | S | .github | Pin Actions to commit SHAs. | `actions/checkout`, `setup-java`, `setup-node`, `setup-gradle`, `upload-artifact` use full SHAs; Dependabot `github-actions` keeps them current. Finding S-F-8. Sprint 28. | CI-1 |
| SEC-11 | Task | P1 | Ready | M | content + ingestion + editor | Cap JSON body size and nesting. | Reject oversized `PUT /versions/{id}/articles`, `POST /import-jobs`, and editor saves (Content-Length filter or Jackson `StreamReadConstraints`). Finding S-B-14. Sprint 28. | ING-1, CNT-3 |
...
| --- | --- | --- | --- | --- | --- |
| S-F-1 | Next.js 14.2.5 is EOL (Server Actions SSRF GHSA-89xv-2m56-2m9x). Caddy now pins `Host`; still need a supported Next line. | High | Open | 28 | SEC-1 |
| S-F-5 | No Content-Security-Policy (other browser headers landed). | Medium | Open | 28 | SEC-2 |
| S-F-15 | `/api/docs/*` Swagger/OpenAPI is public, including identity and editor. | Low | Open | 28 | SEC-3 |
| S-B-1 | Unauthenticated `GET /events` leaks login IPs, emails, draft bodies. | High | Open | 28 | SEC-4 |
| S-B-3 | Catalog `POST /versions/{id}/publish` allows any editor / `catalog:write` (no publisher + step-up). | Medium | Open | 28 | SEC-5 |
| S-B-7 | Inter-service `RestClient`s have no connect/read timeouts. | Medium | Open | 28 | SEC-6 |
| S-B-16 | Service tokens never expire. | Info | Open | 28 | SEC-7 |
...
| S-F-8 | GitHub Actions pinned by mutable `@v4` tag, not SHA. | Low | Open | 28 | SEC-10 |
| S-B-14 | No JSON body size / nesting cap on import, article replace, editor save. | Low | Open | 28 | SEC-11 |
...
### Sprint 28: Security review leftovers
**SEC-1**, **SEC-2**, **SEC-3**, **SEC-4**, **SEC-5**, **SEC-6**, **SEC-7**, **SEC-10**, **SEC-11**. Findings [Security review (2026-09-08)](#security-review-2026-09-08).

Close the leftover High/Medium items from the 2026-09-08 scan. Land **SEC-4 and SEC-5 first** (unauthenticated audit reads; catalog publish is publisher-only), then **SEC-1** (Next 15). CSP (SEC-2) follows the Next upgrade. Swagger lock-down, RestClient timeouts, token expiry, Action SHA pins, and JSON body caps complete the sprint.

**SEC-9** lands in Sprint 26 with PLAT-5 (one allow-list, not eight copies). **SEC-8** stays Ideas.

Owners: gateway-web (SEC-1, SEC-2), audit-service (SEC-4), catalog + identity (SEC-5), all services (SEC-6), identity (SEC-7), Caddy (SEC-3), GitHub Actions (SEC-10), content + ingestion + editor (SEC-11). One L only (SEC-1).
