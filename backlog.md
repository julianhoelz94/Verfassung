...
| CNT-4 | Task | P0 | Ready | M | content-service | Reject writes to a published version. | `PUT /versions/{id}/articles`, `PATCH /articles/{id}`, `PATCH /nodes/{id}`, and `POST /versions/{id}/restructure` call catalog `GET /versions/{id}` (CAT-4). If `publicationStatus == published`, return 409 with code `version_published` and make **no** SQL writes. If catalog is unreachable, return 503 and make no writes. Stub/fake the catalog client in `ContentApiTest`; do not share catalog’s database. Finding [Q-2](#q-2-mutating-published-law). Sprint 23. | CNT-3, CAT-4, GOV-6 |
...
| QLT-9 | Task | P1 | Ready | S | editor-service | Remove QLT-5 in-place publish leftovers. | After ED-4: `EDITOR_PUBLISH_PUBLIC` gone from `env/*.env.example`, `docker-compose.yml`, and `EditorPublishProperties`. `rg updateArticle` in editor-service has no publish loop. Finding [Q-2](#q-2-mutating-published-law). Sprint 23 (same change as ED-4 if already done there — close this ID together). | ED-4, QLT-5 |
...
| --- | --- | --- | --- | --- |
| <a id="q-1-public-write-apis"></a>Q-1 | Public write APIs: Caddy proxies `/api/catalog*`, `/api/content*`, `/api/audit*`, `/api/search*` with no auth; controllers accept unauthenticated PUT/PATCH/POST (replace articles, publish version, append audit, reindex). | Failing | 22 | GOV-6, OPS-9, IDN-14 |
| <a id="q-2-mutating-published-law"></a>Q-2 | Publish patches existing public articles in place (`EditorService.publish` + `EDITOR_PUBLISH_PUBLIC`). Violates ARCH-2 / CNT-1 “snapshot rows immutable.” | Failing | 23 | ARCH-6, CAT-4, CNT-4, ED-4, QLT-9 |
...
| <a id="q-14-invite-and-reset-tokens-in-the-query-string"></a>Q-14 | Admin invite/reset tokens in `?invited=` / `?reset=` (`account/actions.ts` redirect). Leak via Referer/history/logs. | Margin | 22 | IDN-15 |

## Suggested Sprint Breakdown

Pull **only Ready P0 tasks** listed under each sprint. Do not re-open Done IDs.

Recommended order after Sprint 15: **22 → 23 → 19 → 20 → 21 → 16 → 17 → 18 → 24 → 25 → 26 → 27**.

P0 quality (lock writes, then immutable publish) lands **before** the redesign so UI-38 does not restyle an in-place publish that ED-4 removes. Reader/search/discovery (16–18) still wait for the new shell (19–21). Content-model, amendment, platform, and search-language work follow. Findings: [Quality review (2026-09-06)](#quality-review-2026-09-06).
...
Concept: `ui-concept.md` §4.5, 4.8–4.9; screenshots `board-compare.png`, `board-editor.png`. Compare with sticky selector bar, phone tabs and two-column cards; editor workspace with workflow steps, three panels and sticky action bar, plus role landing lists backed by the new `GET /edit-sessions` (ED-3); admin pages on `DataList`. Publish UX follows ED-4 (new version), not QLT-5. Ends with removing the alias block and old classes, re-baselining UI-14 visuals, and an axe/Lighthouse pass. Owners: gateway-web + editor-service (one read endpoint).

### Sprint 22: Lock the public write surface — next
**GOV-6**, **OPS-9**, **IDN-14**, **IDN-15**. Findings [Q-1](#q-1-public-write-apis), [Q-14](#q-14-invite-and-reset-tokens-in-the-query-string).

Anonymous GET on catalog/content/amendment/search stays. Every mutating method on catalog, content, audit, search, and ingestion returns 401 without a Bearer that identity `/me` accepts, and 403 without the right role. Caddy additionally rejects those methods when `Authorization` is missing. Ingestion/search automation gets rotatable machine tokens (IDN-14). Invite and reset tokens leave the query string.
...
**PLAT-5**, **SRC-8**, **UI-40**, **UI-41**. Findings [Q-6](#q-6-eight-cloned-microservices), [Q-7](#q-7-english-sentence-splitter-on-german-law), [Q-9](#q-9-search-uses-simple-tsvector), [Q-12](#q-12-axe-contrast-disabled).

One Gradle `platform` module; delete eight `CorrelationIdFilter` / `NotFoundException` copies. Search `tsv` uses `german` for `de`. Compare diffs outline nodes before `[.!?]`. Axe color-contrast enabled.
...
### Later / Ideas
SRV-5, **SRV-7 (MCP, postponed)**, **GW-2**, IDN-11, ING-4–5, SRC-4, PLAT-3, QLT-2, QLT-7, DEP-2–6, **GOV-7** (fold audit only if it stays one endpoint after GOV-6).

## Open Product Questions
- Published constitutional versions are immutable snapshots (ARCH-6 / ED-4). Official errata, if ever needed, are a new version — not an in-place PATCH.
