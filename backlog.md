...
- **Snapshot chain (ARCH-7, Sprints 29–33):** one linear published-snapshot chain per constitution; hop `initial` / `legal_amendment` / `official_errata` / `editorial_correction`. **Decided (ARCH-8 ADR 0005; Sprints 35–39 implement):** two axes — legal versions vs editorial revisions of each law and each change record. See [`docs/adr/0005-two-axis-versions.md`](docs/adr/0005-two-axis-versions.md), [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) (**Sprint UI reference**) and [legal vs editorial chain](/Users/julianhoelz/.cursor/projects/Users-julianhoelz-Documents-Verfassung/canvases/legal-vs-editorial-chain.canvas.tsx). ARCH-6 still forbids PATCH of a published tree.
- **Amendments (ARCH-7):** an amendment is a curated legal event (which law changed the text, and how). It is not auto-created from an editorial publish. Change rows can target a whole article or any node. Version ids on an amendment are optional (historical laws may be recorded without storing every old snapshot). **Decided (ARCH-8):** same objects are **change records** (title, comment, documents); no kind enum for amending-law vs errata.
- **Covert histories:** (1) editorial snapshot hops are omitted from public version lists and the public timeline; (2) each amendment has its own linear revision chain, staff-only; the public sees the latest published revision. **Decided (ARCH-8):** editorial hops hang off the legal version they correct, not the constitution tip; `needs_review` is staff-only until a re-pin is published.
...
| ARCH-6 | Story | P0 | Done | M | docs + catalog + content + editor | Published versions are immutable snapshots. | Answers the open product question in this file. Write `docs/adr/0002-immutable-published-versions.md`: a `publication_status = published` catalog version never has its content tree rewritten; corrections and errata are a **new** catalog version (optional `provenance = official` plus a note), never `PATCH`/`PUT` on the old `versionId`. ARCH-2 stays the logical rule; QLT-5 in-place rewrite is deprecated and removed by ED-4. Finding [Q-2](#q-2-mutating-published-law). Sprint 23. ARCH-7 adds hop kinds so not every successor is a public amendment. | ARCH-2, QLT-5 |
| ARCH-7 | Story | P0 | Done | M | docs | Legal amendments ≠ editorial version control. | New `docs/adr/0004-amendments-and-editorial-history.md` (Accepted). Do not reopen ARCH-4/ARCH-6/AMD-5. The ADR must state: (1) public timeline = curated amending laws and official gazette errata, never a dump of publishes; (2) one linear snapshot chain per constitution, no branches/merges; (3) hop kinds `initial` / `legal_amendment` / `official_errata` / `editorial_correction`; (4) editorial hops are listed only to editor/reviewer/publisher/admin; public “latest” is the chain tip so typo fixes reach readers; old permalinks keep ARCH-6 bytes; (5) each amendment has a linear revision history, staff-only; public GET returns the latest published revision; (6) AMD-5 tree-diff may *suggest* change rows, it must not persist an amendment on every ED-4 publish; (7) publish of article text may append only to the chain tip (409 if the session’s version already has a successor). Update ADR 0002 consequences (drop “every successor records an amendment”). Update `docs/CODEMAPS/data.md`, `backend.md`, `frontend.md`, `docs/editorial-roles.md`, `docs/design/role-link-trees.md`, `docs/design/ui-concept.md` §4.8 plus a new § for `/editor/amendments`. Sprint 29. Superseded in part by ARCH-8 (Sprints 34–39). | ARCH-4, ARCH-6, AMD-5, ED-4 |
| ARCH-8 | Story | P0 | Done | M | docs | Two-axis legal × editorial versions. | Write `docs/adr/0005-two-axis-versions.md` (Accepted). Supersede ADR 0004 points 2–4; keep 1 and 5–7. Must state: (1) two axes — linear **legal** versions (public chips) vs Wikipedia-style **editorial** revision history of each legal object and each change record (no branches/merges on either axis); (2) catalog `hop_kind` is only `initial` / `legal` / `editorial_correction` — do not keep `official_errata` as a kind, badge, or hop enum; (3) a change record is title + **comment** + **documents**, not “amending law vs errata”; (4) each published change-record revision pins two catalog snapshot ids; `needs_review` when pins ≠ live editorial tips of those legal versions; do not auto-rewrite quotes; public GET stays on the last published revision until staff publish a re-pin; (5) each law snapshot has at most one change-record **forward** and at most one **backward**; the live hop always runs on current editorial tips (`needs_review` when published pins lag); retired snapshots keep a dashed pin; (6) editorial publish appends only to that legal version’s editorial tip; legal publish appends only from the legal tip’s editorial tip; public “latest” is the editorial tip of the legal tip; ADR 0002 permalinks unchanged. Update ADR 0002/0004 status lines, keep [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) aligned, CODEMAPS INDEX/data/backend/frontend, `docs/editorial-roles.md`, `docs/design/role-link-trees.md` and ui-concept copy (“Legal changes”). Design source: [legal vs editorial chain](/Users/julianhoelz/.cursor/projects/Users-julianhoelz-Documents-Verfassung/canvases/legal-vs-editorial-chain.canvas.tsx). UI contract: [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) § Sprint UI reference. Sprint 34. | ARCH-7, CAT-5, AMD-7, ED-6 |
...
| SRV-4 | Story | P1 | Done | M | docs + editor-service | Domain event names for publish, amendment, reindex. | `docs/adr/0003-domain-events.md` defines JSON payloads (no broker): `version.published` (`sourceVersionId`, `newVersionId`, `constitutionId`, `actorId`), `amendment.recorded` (`transitionId`, `sourceVersionId`, `targetVersionId`), `search.reindex-requested` (`versionId`). Field names are stable; consumers must be idempotent. Implemented on the editor outbox in PLAT-6, not a message bus. Finding [Q-8](#q-8-swallowed-reindex--no-outbox). Sprint 27. | SRV-3, ARCH-4, ED-4 |
...
| OPS-9 | Task | P0 | Done | S | infra/caddy | Caddy refuses anonymous writes on the public edge. | In `infra/caddy/Caddyfile`, POST/PUT/PATCH/DELETE under `/api/catalog*`, `/api/content*`, `/api/audit*`, `/api/search*`, and `/api/ingestion*` require an `Authorization` header (401 if missing). Caddy does **not** validate the token — GOV-6 does. GET (and HEAD) on catalog, content, amendment, and search stay open. `/api/identity*` and `/api/editor*` unchanged (already session/Bearer). A curl without `Authorization` to `PUT /api/content/articles/{id}` through `:80` returns 401 before the service body. Finding [Q-1](#q-1-public-write-apis). Sprint 22. | GOV-6 |
...
| ING-3 | Story | P1 | Done | S | ingestion-service | Attach sources/citations on import. | `ImportRequest.sourceUrl`, `gazetteReference`, and `languageCode` are forwarded to catalog `constitution_sources` on the created version; an import with all three set shows them in `Provenance` on the version page; test asserts the catalog source row. Sprint 15. | ING-1, CAT-1 |
| ING-4 | Story | P1 | Ideas | L | amendment-service | Amendments as change sets → new version. | Import path that records a change set and materializes a new content snapshot. Editorial recording of laws is AMD-8 + UI-43 (not AMD-5 persist). Keep this ID until an ingestion-specific change-set upload exists. | DB-5, AMD-5, ED-4, AMD-8 |
| ING-5 | Story | P2 | Ideas | L | later | Translation import. | Out of Sprint 1–2. | ARCH-5 |
| ING-6 | Story | P1 | Done | M | ingestion-service | Import request accepts a content tree. | `ImportArticle` gains optional `nodes: List<ImportNode>` (`kind`, `label?`, `title?`, `body?`, `children`); flat `body` remains the fallback. `ImportRequest` gains optional `outline` in the CAT-3 `PUT /constitutions/{id}/content-outline` shape, applied after the constitution is created. Nodes are written through the existing content write API; a `kind` missing from the outline fails the job with `import_errors` code `UNKNOWN_KIND` and no catalog write. Tests: flat import unchanged, tree import, unknown-kind failure. Sprint 15. | ING-1, CAT-3, CNT-3 |
| ING-7 | Task | P1 | Done | S | ingestion-service + repo | Second country as an importable JSON fixture. | A checked-in fixture (e.g. US, outline `article → section`) under `services/ingestion-service/src/test/resources/fixtures/` imports successfully through UI-21 and is used by the ING-6 tree test. Sprint 15. | ING-6, UI-21 |
...
| UI-10 | Story | P1 | Done | L | gateway-web | Structured text diff. | Changed articles show node- or sentence-level additions/removals while retaining authoritative AMD-3 change records and complete before/after text. Whole-article and sub-article changes render consistently. | UI-9, AMD-3, UI-11 |
...
| UI-14 | Story | P1 | Done | M | gateway-web + CI | Frontend journey and visual tests. | Playwright covers browse → article, search, linear compare, login/logout, edit/review/publish; axe runs on critical routes; visual regression covers narrow/wide public and editor layouts; failures run in CI. | CI-2, UI-7, QLT-5 |
| UI-15 | Story | P1 | Done | M | gateway-web | Discovery, print, and sharing. | `generateMetadata` on country, version, article, compare, timeline, and search routes. Title pattern `Article 1 · Basic Law 1949 · Germany — Constitution Atlas`; description is the first ~160 chars of the article body. `metadataBase` in `layout.tsx` and `alternates.canonical` built from a new `PUBLIC_BASE_URL` in every `env/*.env.example`; compare canonical keeps `from`/`to` in canonical order (helper in `lib/compare.ts`). Open Graph title/description on all public routes. `@media print` in `globals.css` hides header, breadcrumbs, and `.print-hide`; compare grid prints as one column. Sprint 18. | UI-2, UI-9 |
...
| UI-21 | Story | P1 | Done | M | gateway-web | Admin import page. | New `lib/ingestion-api.ts` (`createImportJob`, `getImportJob`) and `ingestionBaseUrl()` next to the other `*BaseUrl()` helpers; Caddy route `/api/ingestion*` already exists. `app/admin/import/page.tsx` + `actions.ts` guarded by `canVisitAdmin` like `app/admin/users`; admin menu link added in `lib/nav.ts` (`nav.test.ts` updated). Form accepts pasted JSON or a `.json` file, validates required fields with a type guard, posts, and redirects to `app/admin/import/[jobId]/page.tsx`, which shows `status`, `errors`, and on success a link to `/countries/{isoCode}/versions/{versionId}`; refreshes via `<meta httpEquiv="refresh">` while `running`. Uses `ui.tsx` primitives. An admin can go from an empty database to a browsable second country without leaving the browser. Sprint 15. | ING-6, UI-17, UI-13 |
| UI-22 | Story | P1 | Done | S | gateway-web | Previous / next article navigation. | Article page calls `listAllArticles(versionId)` and renders `<nav aria-label="Article navigation">` with previous, table-of-contents, and next links; hidden at the ends. Logic lives in pure `neighborsOf(articles, currentId)` in `lib/article-nav.ts` with first/last/middle unit tests; `ArticleNav` component is reused by `VersionReader` for in-place expanded articles. Sprint 16. | UI-2, UI-19 |
| UI-23 | Story | P1 | Done | M | gateway-web + amendment | Article history page. | `/countries/[code]/articles/[articleNumber]` lists, in canonical version order, every version's text of that article (shared `ConstitutionText` + `Provenance`) with a "Compare with previous" link, and between versions the AMD-4 changes (`change_type`, `changed_on`, `effective_on`, citation). Linked from the article page header ("History of Article N") and from every change row on the compare page. One `listAllArticles(v.id, true)` per version, no per-article calls. Sprint 16. | UI-22, AMD-4, UI-9 |
| UI-24 | Task | P1 | Done | S | gateway-web | Global search box. | `SiteHeader` has `<form role="search" action="/search" method="get">` with one labelled `<input type="search" name="q">` (44px target); the same form sits above the country list on `/`. `a11y-shell.test.ts` covers it. Sprint 17. | SRC-1, UI-7 |
| UI-25 | Task | P1 | Done | S | gateway-web | Search within a version. | Version page has a search form posting to `/search` with hidden `versionId` and `country`; result links keep the version. Sprint 17. | UI-24, SRC-2 |
| UI-26 | Task | P1 | Done | S | gateway-web | Sitemap and robots. | `app/sitemap.ts` lists every country, version, and article URL (via `listCountries`, `getCountry`, `listAllArticles`); `app/robots.ts` disallows `/editor`, `/admin`, `/account`, `/login`, `/reset`, `/invite`, and `/search?`. Both use `PUBLIC_BASE_URL`. Sprint 18. | UI-15 |
| UI-27 | Task | P1 | Done | S | gateway-web | Correct text language and readable dates. | `ConstitutionText` root carries `lang={version.languageCode}` so German text is announced in German while the shell stays `en`. `lib/format-date.ts` `formatDate(iso)` uses `Intl.DateTimeFormat` (`dateStyle: 'long'`) wrapped in `<time dateTime>`; replaces raw ISO dates in `Provenance`, timeline, compare, and search hits. One unit test. Sprint 18. | UI-11, UI-12 |
| UI-28 | Story | P1 | Done | S | gateway-web + catalog | Informative home page. | `CountrySummary` gains `latestVersionLabel`, `latestEffectiveDate`, `versionCount` (one aggregate query in `CatalogRepository.kt`; `catalog-countries.json` fixture and `CatalogApiTest` updated). `/` renders a `Card` per country with those values and a "Compare latest two" link when two or more versions exist. Sprint 18. | CAT-2, UI-13 |
| UI-29 | Task | P0 | Done | S | gateway-web | Design tokens and typography reset. | Design: `docs/design/ui-concept.md` §6, `docs/design/tokens.css`. Copy `tokens.css` into `globals.css` `:root`; add a temporary alias block mapping old names (`--ink`, `--muted`, `--paper`, `--line`, `--accent`, `--added`, `--removed`, `--changed`, `--surface`, `--danger`, `--radius`) to the new tokens so nothing breaks; body uses `--font-ui`, `.text-column`/`.constitution-body` use `--font-text` at `--text-md`/`--leading-reading`; focus ring is `--color-focus`; `.page`, `.page-narrow`, `.gutter` layout classes replace `main`/`main.wide` max-widths. `npm run build` passes with no visual regressions beyond fonts/colours. Sprint 19. | UI-13 |
| UI-30 | Task | P1 | Done | M | gateway-web | New shared primitives. | Design: `docs/design/ui-concept.md` §5; classes in `docs/design/prototype/prototype.css`. Add to `app/components/ui.tsx` (or siblings): `Badge` (`accent`/`added`/`removed`/`changed`/`info`/default, replaces `.tag`), `Chip`, `PageHeader` (breadcrumbs, eyebrow, title, meta, actions), `SiteFooter`, `Pager`, `DataList`/`DataRow` (table ≥ 640px, cards below), `Button` gains `ghost` variant and `size="sm"`. Each has explicit props types and a unit test where logic exists. Sprint 19. | UI-29 |
| UI-31 | Story | P0 | Done | M | gateway-web | Responsive top bar with role-aware menu. | Design: `docs/design/ui-concept.md` §3, `docs/design/role-link-trees.md`. `lib/nav.ts` adds `menuSections(user)` returning ordered sections (identity, primary, Editorial, Admin, account) per the link trees; `primaryNavLinks` keeps the desktop bar. New `SiteHeader`: brand, centred primary nav (≥ 1024px), pill search (≥ 640px), Log in button or avatar/☰ opening a `MenuDisclosure` client component (Escape and outside click close, focus returns to the trigger, `aria-expanded`). Phone search row under the bar; on version/article pages it carries `versionId` (UI-25). Identity-outage status line kept (UI-6). `nav.test.ts` covers the matrix in `role-link-trees.md`; `a11y-shell.test.ts` updated. Sprint 19. | UI-29, UI-30, UI-17 |
| UI-32 | Task | P1 | Done | S | gateway-web | Routes required by the link trees. | `/about` static page (sources, verification labels, accessibility statement); `/admin` index page with three cards (Users, Outlines, Import) guarded by `canVisitAdmin`; footer links About · Sources · Accessibility. Both use `PageHeader` and `.page-narrow`. Sprint 19. | UI-30, UI-31 |
| UI-33 | Story | P1 | Done | M | gateway-web | Home and country pages in the new design. | Design: `docs/design/ui-concept.md` §4.1–4.2, `docs/design/prototype/home.html`. Home: hero with large search, four stats, `card-grid` of country cards (ISO badge, title, Latest badge, counts, Read latest / Compare latest two), "Recently changed" list from the latest versions' amendments (silent degrade). Country: constitution cards with version `Chip` row in canonical order and Timeline / Compare actions. Builds on UI-28 data. Sprint 20. | UI-30, UI-31, UI-28 |
| UI-34 | Story | P0 | Done | L | gateway-web | Version reader and article page in the new design. | Design: `docs/design/ui-concept.md` §4.3–4.4, `docs/design/prototype/version.html`. `reader` grid (TOC column ≥ 1024px), sticky `Toolbar` with **Contents** disclosure (< 1024px) and a `Segmented` control replacing the range slider (labels from `depthStopLabels`), `Toc` client component with scroll-spy `aria-current`, `.text-column` with `.para` number gutter, article footer (Permalink, History, unchanged/changed badge), section pager. Article page uses `.page-narrow`, `PageHeader`, `Provenance`, `lang`, and the UI-22 pager. Existing behaviour of `VersionReader` (expand in place, `aria-expanded`) is preserved; tests in `outline.test.ts` unchanged. Sprint 20. | UI-29, UI-30, UI-31, UI-19, UI-22 |
| UI-35 | Story | P1 | Done | M | gateway-web | Timeline and search in the new design. | Design: `docs/design/ui-concept.md` §4.6–4.7. Timeline as a vertical list (date column ≥ 640px, stacked below) with version cards and Compare with previous / Read actions. Search: query in the `h1`, filter sidebar ≥ 1024px and a **Filters** disclosure below, result cards with highlighted snippet (SRC-6) and country name (SRC-7), pager. Sprint 20. | UI-30, UI-31, UI-16 |
| UI-36 | Task | P1 | Done | S | gateway-web | Account, login, invite, reset in the new design. | Design: `docs/design/ui-concept.md` §4.10. Each page is `.page-narrow` with one centred `Card`, `PageHeader`, `Alert` above the form, footer visible; no layout branching by width. Sprint 20. | UI-30, UI-31 |
| UI-37 | Story | P1 | Done | M | gateway-web | Compare in the new design. | Design: `docs/design/ui-concept.md` §4.5, `docs/design/prototype/compare.html`. Sticky `compare-bar` (one row ≥ 640px), summary badges with "Show all", `Tabs` client component (**Changes / From / To**, `role="tablist"`, hidden ≥ 640px), hops as collapsed `<details class="hop">`, per-article `compare-grid` with two `compare-col` cards (column head with version label + badge), `<ins>`/`<del>` styled from the semantic pairs so UI-10 output renders without further CSS. Keyboard flow and deep links from UI-9 preserved; `compare.test.ts` unchanged. Sprint 21. | UI-30, UI-31, UI-9 |
| UI-38 | Story | P1 | Done | L | gateway-web + editor-service | Editor workspace and admin pages in the new design. | Design: `docs/design/ui-concept.md` §4.8–4.9, `docs/design/prototype/editor.html`, `docs/design/role-link-trees.md`. Editor: `WorkflowSteps` pills, `workspace` grid (articles panel with filter and draft/new badges; editor panel; session panel with preview), sticky bottom `action-bar` (< 1024px), role-dependent actions (Save/Submit, Approve, Publish). Publish follows ED-4 (new catalog version + copied tree), not QLT-5 in-place rewrite; success copy names the new version, not “text for this version was updated.” Landing without a session shows Open a session, Load a session, and — via ED-3 — My sessions / Review queue / Ready to publish lists per role. Admin Users, Outlines, Import use `PageHeader` + `DataList`. Sprint 21 (after Sprint 23). | UI-30, UI-31, UI-32, ED-3, ED-4, IDN-8 |
| UI-39 | Task | P1 | Done | S | gateway-web + CI | Redesign verification and clean-up. | Remove the UI-29 alias block and every old class (`.tag`, `.compare-form`, `.hop h3`, `main.wide`, …) once no usages remain (`rg` shows none); re-baseline UI-14 visual snapshots at 390, 820 and 1440px; axe passes on home, version, article, compare, search, login, editor, admin users; Lighthouse accessibility ≥ 95 on version and compare pages. Sprint 21. | UI-14, UI-33, UI-34, UI-35, UI-36, UI-37, UI-38 |
| UI-40 | Task | P1 | Done | M | gateway-web | Diff legal units, not English prose sentences. | `apps/gateway-web/lib/text-diff.ts`: `alignNodes` remains the first axis. `diffText` / `splitSentences` using `(?<=[.!?])` run **only** on a concatenated leaf body that has no child nodes. When both sides have `children`, diff by node (`kind` + `label`/`number`), then word-diff leaf `body`. Unit tests: (1) two trees that differ by one `sentence` node do not split on `Abs.`/`Art.`; (2) a single unsplit paragraph still word-diffs. `text-diff.test.ts` covers both. Finding [Q-7](#q-7-english-sentence-splitter-on-german-law). Sprint 26. | UI-10, ARCH-5 |
| UI-41 | Task | P1 | Done | S | gateway-web | Axe color-contrast is on. | In `e2e/a11y.spec.ts`, remove `{ rules: { 'color-contrast': { enabled: false } } }`. Fix any violations on the routes already visited (home, search, login, DE version, article, compare, editor after sign-in). A single route may be skipped only with a comment pointing at a follow-up ID; no global disable. Finding [Q-12](#q-12-axe-contrast-disabled). Sprint 26. | UI-7, UI-14 |
| UI-42 | Story | P0 | Done | M | gateway-web | Public timeline is laws, not publishes. | `app/countries/[code]/timeline/page.tsx` lists AMD-9 published amendments (`legal_amendment` and `official_errata`) for that constitution, ordered by `enactedOn` NULLS LAST. Do **not** iterate catalog versions and `listAmendments(versionId)` as if every hop were a law. Empty copy: “No amending laws are recorded for this constitution.” (valid). `official_errata` uses a distinct Badge (e.g. `info`) and heading that says errata, not “amendment”. Home “recently changed” (`lib/reading.ts`) uses the same published-amendment list. `e2e` mock-api and `timeline.test.ts` updated. Sprint 31. | UI-5, AMD-9, ARCH-7 |
| UI-43 | Story | P0 | Done | L | gateway-web | User-friendly amendment list editor. | New routes (login required, `canVisitEditor`): `/editor/amendments` constitution `<Select>` + `DataList` of that constitution’s amendments (staff list: draft/published/withdrawn) with title, kind, enacted date, status Badge, and **Add amending law**. `/editor/amendments/[id]` is a form, not JSON: title, kind (legal amendment / official errata), citation (`sourceReference`), enacted/effective dates, summary textarea, optional source/target version `<Select>` (public + staff versions from CAT-5 `listing=all`). **Changes** is an editable table: article number (combobox from `listAllArticles` of the target or latest version, plus free text), `added`/`changed`/`removed`, note. Add/remove rows; 44px controls; errors announced. Actions: **Save draft** (`POST` create or `/revisions`), **Publish law** (publisher/admin, GOV-8), **Withdraw** (publisher, if published). Reviewer: read-only. Uses `PageHeader`, `Card`, `DataList`, `Button`, `Input`, `Select`, `Alert`. Client: `lib/amendment-editor-api.ts` (session cookie → amendment-service via server actions, same pattern as `editor/actions.ts`). Empty constitution: one card “No amending laws yet” + Add. Keyboard-complete. Sprint 32. | UI-ED-2, AMD-8, AMD-9, UI-13, UI-38 |
| UI-44 | Story | P1 | Done | M | gateway-web | Staff-only amendment revision history. | On `/editor/amendments/[id]`, a `panel-side` (or `<details>` on phone) lists AMD-9 revisions oldest→newest: date, actor email or id slice, “Draft” vs “This is public”. Clicking a past revision shows that payload **read-only**. **Restore as new draft** copies those fields into a new tip revision (still linear — never forks). Anonymous/viewer hitting the revisions API get 401/403; the panel is not rendered. Sprint 33. | UI-43, AMD-9 |
| UI-45 | Task | P1 | Done | M | gateway-web | Suggest changes from two snapshots. | On the amendment form, **Fill from two versions** (secondary button) opens a dialog: source/target `<Select>`, then `POST /amendments/suggest`. Suggested rows merge into the changes table (editor can delete/edit). Does not save until Save draft. Hidden when content-service is down (`Alert`). Sprint 33. | UI-43, AMD-10 |
| UI-46 | Story | P0 | Done | M | gateway-web | Publish dialog: hop kind + link a law. | Editor session action bar Publish (`app/editor/page.tsx`, `actions.ts`) is no longer a bare POST. Publisher must choose hop kind: “Amending law” / “Official errata” / “Fix transcription (not a law)”. Legal/errata: required `<Select>` of draft or published amendments from AMD-9 staff GET (seed DE is enough until UI-43). Editorial: no amendment select; help text that this hop will not appear on the public timeline. `publishAction` sends `hopKind` + `amendmentId` to ED-6. Success alert names the new version and, for legal hops, the law title. Sprint 32 adds the “Record a law first” link to `/editor/amendments`. Sprint 31. | ED-6, UI-38, AMD-9 |
| UI-47 | Story | P1 | Done | M | gateway-web | Staff snapshot history (linear). | `/editor/history?constitutionId=` (also from country page **Version history** for editorial roles only). `DataList` of CAT-5 `listing=all` published versions oldest→tip: label, hop kind Badge, listing, date, link to reader. Editorial hops labelled “Staff only”. No branch UI. `nav.ts` editorial link “Snapshot history”. `nav.test.ts` matrix. Sprint 33. | CAT-5, UI-31, ARCH-7 |
| UI-48 | Task | P0 | Done | S | gateway-web | Editorial nav: Amendments. | `editorialLinks` + `docs/design/role-link-trees.md`: editors/publishers/admins get `/editor/amendments` (“Amending laws”). Reviewer: same href, read-only (UI-43). Publish dialog (UI-46) gains “Record a law first”. `nav.test.ts`. Sprint 32. | UI-31, UI-43, UI-46 |
| UI-49 | Story | P0 | Done | M | gateway-web | Public version chips hide editorial hops. | Country page, compare `<Select>`, version reader “latest”, sitemap: use CAT-2 public listing only. “Read latest” / home latest uses the **chain tip** (`GET` constitution versions `listing=all` is staff-only — catalog must expose public `latestVersionId` as the newest published row in predecessor order, including a staff tip). If the tip is `editorial_correction`, public still reads that text at `/countries/{code}/versions/{tipId}` (200) but the chip row does not list it as a named historical version. `lib/reading.ts` / `getCountry` types gain `hopKind`/`listing`/`predecessorVersionId` when present. Tests in `reading` or page unit tests. Sprint 31. | CAT-5, UI-1, UI-33 |
| UI-50 | Story | P1 | Done | M | gateway-web | Compare path follows curated laws. | `/countries/[code]/compare` hop list is published amendments between the two public versions (AMD-9), not every catalog successor. Editorial hops in between are skipped in the public hop UI. Side-by-side ends still use the selected version snapshots (UI-4). Copy when no laws exist between two public versions: compare text still works; hop list empty. `lib/compare.ts` + compare tests. Sprint 31. | UI-4, UI-9, AMD-9, UI-42 |
| UI-51 | Task | P1 | Done | M | gateway-web | Journeys: record a law, typo fix, timeline. | Playwright (mock API updated in `e2e/mock-api.mjs`): (1) editor records an amending law, publisher publishes it, anonymous timeline shows that title; (2) publisher publishes an `editorial_correction` hop, anonymous timeline unchanged, staff history shows the hop; (3) axe on `/editor/amendments`. Extend `e2e/journeys.spec.ts` / `a11y.spec.ts`. Sprint 33. | UI-14, UI-42, UI-43, UI-46, UI-47 |
| UI-52 | Story | P0 | Ready | M | gateway-web | Next-legal-change workspace; transcription is a separate session. | Replace UI-46’s hop-kind Publish dialog. **Record the next legal change** opens a session on the legal tip (P2): article editor + change-record sidebar (title, comment, documents). One publish: **Publish new legal version** (`hopKind=legal`), blocked until text is approved and the instrument is complete; mints the new chip and pins the record. **Correct this text** on a chip is a different session (P3) whose only publish is **Publish transcription**. Do not offer both jobs in one dialog. `publishAction` sends ED-7 fields. Success alert names the new snapshot and, for legal hops, the change-record title. Mocks: [`editor-constitution.html#legal`](docs/design/prototype/editor-constitution.html#legal) (P2), [`#correct-1949`](docs/design/prototype/editor-constitution.html#correct-1949) (P3). Sprint 37. | UI-46, ED-7, UI-43 |
| UI-53 | Story | P0 | Ready | M | gateway-web | Public chips are legal identities. | Country page, compare `<Select>`, version reader “latest”, sitemap, home latest: CAT-6 public list — one chip per legal version; href/read uses `currentVersionId` (editorial tip). Do not list editorial snapshots as extra chips. Permalink `/countries/{code}/versions/{snapshotId}` stays 200 for staff hops (ADR 0002). Types in `lib/reading.ts` / `getCountry`: `legalVersionId`, `currentVersionId`, `hopKind` `initial`\|`legal`\|`editorial_correction`. Tests in `reading` or page unit tests. No two-axis public mock — do not copy `prototype/home.html` or `version.html`. Legal identities on [`#timeline`](docs/design/prototype/editor-constitution.html#timeline). Sprint 38. | UI-49, CAT-6, ARCH-8 |
| UI-54 | Story | P0 | Ready | M | gateway-web | Timeline: comment + documents, no errata kind. | `/countries/[code]/timeline` lists AMD-11 published change records: title, comment, document links (URL and/or archived file label), dates. Drop `official_errata` Badge and “errata” heading. Empty copy: “No legal changes are recorded for this constitution.” Home “recently changed” uses the same list. Mock-api + timeline tests. Hop-card visual language: [`#timeline`](docs/design/prototype/editor-constitution.html#timeline). Do not copy `prototype/compare.html`. Sprint 38. | UI-42, AMD-11, ARCH-8 |
| UI-55 | Story | P1 | Ready | M | gateway-web | Staff snapshot history (two axes). | `/editor/history` grouped by legal version (chip label). Nested list of editorial hops for that law (Wikipedia comments). Change records sit **between** adjacent legal groups (title + comment; `needs_review` Badge if set). Newest editorial revision first within a group. No branch UI. `nav.ts` copy may stay “Snapshot history”. `nav.test.ts`. Mock: [`#timeline`](docs/design/prototype/editor-constitution.html#timeline) (public versions as nodes, legal changes as edges, collapsed History). Sprint 39. | UI-47, CAT-6, AMD-11, ARCH-8 |
| UI-56 | Story | P0 | Ready | M | gateway-web | Quote-review inbox. | `/editor/amendments` filters include **Needs review**. Editorial nav badge = `needs_review` count (0 → no badge). Opening a flagged record: three columns — source public version (last reviewed beside latest if that chip moved) | the instrument and quoted rows | target public version; **Confirm quotes** (append revision, same change rows, pins = live tips) or edit quotes/documents then publish (publisher). Public GET unchanged until publish. Reviewer: read-only. Mocks: [`#hop-1956`](docs/design/prototype/editor-constitution.html#hop-1956) (stale source), [`#hop-2022`](docs/design/prototype/editor-constitution.html#hop-2022) (stale target), [`#hop-1994`](docs/design/prototype/editor-constitution.html#hop-1994) (in sync). Sprint 39. | UI-43, UI-44, AMD-11, ED-7 |
| UI-57 | Story | P0 | Ready | M | gateway-web | Change-record form: comment + documents. | `/editor/amendments` and `[id]`: drop kind `<Select>`. Required **comment**; **documents** as a list of URL and/or uploaded/archived file + optional label (add/remove rows). This is inbox, quote review, and P1 (instrument between two existing chips). Creating the **next** public version is UI-52 / P2 (sidebar on the article session), not “Add a legal change” as a standalone hop with one end. Nav + `role-link-trees.md`: “Legal changes” (was “Amending laws”). `nav.test.ts`. Mocks: hop cards + **Open record** on [`#timeline`](docs/design/prototype/editor-constitution.html#timeline); form fields on [`#legal`](docs/design/prototype/editor-constitution.html#legal) sidebar and [`#hop-1994`](docs/design/prototype/editor-constitution.html#hop-1994). Sprint 39. | UI-43, UI-48, AMD-11, ARCH-8 |
| UI-58 | Story | P1 | Ready | M | gateway-web | Compare uses legal identities. | Compare selectors are legal chips (UI-53). Hop list is published change records between those identities (AMD-11), not catalog successors. Side-by-side ends use each chip’s `currentVersionId` unless the URL names a snapshot id. Staff-only header warning when a hop on the path is `needs_review`. `lib/compare.ts` + tests. No configured compare mock — do not copy `prototype/compare.html`. Sprint 38. | UI-50, UI-53, AMD-11 |
| UI-59 | Task | P1 | Ready | M | gateway-web | Journeys: correct an old law, quote review. | Playwright (mock API): (1) publisher fixes transcription of 1949 while 2022 is the legal tip — public chips still 1949/2022, “1949” reads new text, timeline unchanged, staff history nests the hop under 1949; (2) that publish flags a change record `needs_review`; publisher confirms quotes — badge clears, public comment unchanged if quotes were confirmed; (3) axe on Legal changes + history. Extend `e2e/journeys.spec.ts` / `a11y.spec.ts`. Screens: [`#correct-1949`](docs/design/prototype/editor-constitution.html#correct-1949) → [`#timeline`](docs/design/prototype/editor-constitution.html#timeline) → [`#hop-1956`](docs/design/prototype/editor-constitution.html#hop-1956). Sprint 39. | UI-51, UI-52, UI-53, UI-55, UI-56 |
...
| CAT-4 | Task | P0 | Done | S | catalog-service | Get one version by id (publication status for writers). | `GET /versions/{versionId}` returns `id`, `constitutionId`, `versionLabel`, `publicationStatus`, `effectiveDate`. 404 if unknown. Does not list unpublished versions on the public country read APIs (CAT-2 unchanged). `CatalogApiTest` covers published, draft (when the caller is allowed — same as today’s publish test user), and unknown. Finding [Q-2](#q-2-mutating-published-law). Sprint 23. | CAT-2 |
| CAT-5 | Story | P0 | Done | M | catalog-service | Linear snapshot chain and hop kind. | New Flyway `V9__version_chain.sql` on `constitution_versions`: nullable `predecessor_version_id` (same-table UUID, no cross-DB FK), `hop_kind TEXT NOT NULL DEFAULT 'initial'` CHECK (`initial`,`legal_amendment`,`official_errata`,`editorial_correction`), `listing TEXT NOT NULL DEFAULT 'public'` CHECK (`public`,`staff`), CHECK that `editorial_correction` ⇔ `listing = 'staff'`. UNIQUE (`predecessor_version_id`) (Postgres allows multiple NULLs). Seed DE: 1949 `initial`/`public` predecessor NULL; 2022 `legal_amendment`/`public` predecessor = 1949. `POST /constitutions/{id}/versions` accepts optional `predecessorVersionId` + `hopKind`; default hop `initial` when predecessor is null, else require hopKind; 409 `not_tip` if that predecessor already has a successor; 409 `unknown_predecessor` if the id is not this constitution. `POST /versions/{id}/publish` unchanged (still publisher-only) but response DTO gains `predecessorVersionId`, `hopKind`, `listing`. Country/list version APIs (CAT-2) return only `publication_status = published` AND `listing = public`. Country `latestVersion` / `latestVersionId` is the **chain tip**: the published row that no other published row names as `predecessor_version_id`, including `editorial_correction`. `GET /constitutions/{id}/versions?listing=all` requires editor/reviewer/publisher/admin and returns the full published chain including staff hops (drafts still omitted). `GET /versions/{id}` returns the new fields for any published version (including staff hops) so “read latest” can use the tip; unpublished still omitted from public country reads. `CatalogApiTest`: second successor of the same predecessor → 409; public list hides an `editorial_correction`; staff list shows it. Sprint 29. | CAT-4, ARCH-7, SEC-5 |
| CAT-6 | Story | P0 | Done | L | catalog-service | Per-legal-version editorial chain. | New Flyway (do not edit V9). Replace constitution-wide `UNIQUE (predecessor_version_id)` with two axes: a stable `legal_version_id` per public chip (first snapshot of that law, or equivalent); `legal_predecessor_version_id` unique among published `hop_kind IN ('initial','legal')`; `editorial_predecessor_version_id` unique among snapshots of the same `legal_version_id`. `hop_kind` CHECK (`initial`,`legal`,`editorial_correction`); migrate `legal_amendment`/`official_errata` → `legal`. `listing = staff` ⇔ `editorial_correction`. `POST /constitutions/{id}/versions`: `editorial_correction` predecessor must be the editorial tip of that legal identity else 409 `not_editorial_tip`; `legal` predecessor must be the editorial tip of the current **legal** tip else 409 `not_legal_tip`. Public CAT-2 list: one row per legal identity with `currentVersionId` = editorial tip and the legal label (e.g. 1949, not the typo-fix snapshot label). `latestVersionId` = editorial tip of the legal tip. `GET /versions/{id}` still 200 for staff hops by id. `GET …/versions?listing=all` returns both axes for staff. `CatalogApiTest`: publish an editorial hop on 1949 after a later legal version exists; second editorial successor of the same tip → 409; public chip count unchanged after that hop; legal hop from a non-legal-tip column → 409. Sprint 35. | CAT-5, ARCH-8 |
...
| CNT-4 | Task | P0 | Done | M | content-service | Reject writes to a published version. | `PUT /versions/{id}/articles`, `PATCH /articles/{id}`, `PATCH /nodes/{id}`, and `POST /versions/{id}/restructure` call catalog `GET /versions/{id}` (CAT-4). If `publicationStatus == published`, return 409 with code `version_published` and make **no** SQL writes. If catalog is unreachable, return 503 and make no writes. Stub/fake the catalog client in `ContentApiTest`; do not share catalog’s database. Finding [Q-2](#q-2-mutating-published-law). Sprint 23. | CNT-3, CAT-4, GOV-6 |
| CNT-5 | Task | P1 | Done | M | content-service | One write model: `content_nodes` only. | Stop INSERT/UPDATE/DELETE on table `articles` from `ArticleRepository` (`replaceForVersion`, `updateText`, `updateNodeTitle`, `restructureKeepingKinds`). New Flyway: drop `articles` **or** replace it with a view over root `content_nodes` (`parent_id IS NULL`). Reads already use `content_nodes` (CNT-3). Move tree flatten/restructure orchestration out of the JDBC repository into `ArticleQueryService` (or a `ContentTreeService`); the repository keeps SQL only. `ContentApiTest` still passes. Finding [Q-4](#q-4-dual-write-articles--content_nodes). Sprint 24. | CNT-3 |
| CNT-6 | Task | P1 | Done | M | content-service | Stable node ids and predecessor links. | `ArticleWrite` / `NodeWrite` accept optional `id`. If present, INSERT uses that UUID; if absent, generate once. `replaceForVersion` must not mint a new id for a node that sent `id`. New nullable `content_nodes.predecessor_id` (same-table UUID, no cross-DB FK). Update the ED-4 copy path to set `predecessor_id` to the source node id while giving the successor new primary ids. `ContentApiTest`: round-trip replace with client ids; omit id → server generates. Finding [Q-5](#q-5-unstable-article-identity). Sprint 24. | CNT-5, ED-4 |
...
| AMD-4 | Task | P1 | Done | S | amendment-service | Read API: amendments by article across versions. | `GET /amendments?constitutionId=&articleNumber=` returns the AMD-2 DTO for every change touching that article in any version of the constitution, ordered by `changed_on`. New Flyway `V{n}__amendment_change_article_number.sql` adds nullable `article_number` to `amendment_changes`; ingestion/editor set it on write. MockMvc test with two versions. Sprint 16. | AMD-3 |
| AMD-5 | Story | P1 | Done | L | amendment-service | Compute added/changed/removed from two content trees. | Add `AmendmentService` (controller must not call `AmendmentRepository` directly — today’s `AmendmentController` does). `POST /transitions` (Bearer from GOV-6) body: `sourceVersionId`, `targetVersionId`, optional `changedOn`, `effectiveOn`, `amendingLawTitle`, `amendingLawCitation`. Load both trees from content HTTP (`includeBody=true`). Match nodes by `predecessor_id` (CNT-6) then by `kind`+`label`/`number`. Persist `version_transitions` + `amendments` + `amendment_changes` with `added`/`changed`/`removed` (body/title differ → `changed`; only in target → `added`; only in source → `removed`). Response is the AMD-2 DTO. MockMvc with a stub `ContentClient`; do not hand-write SQL for the new transition. DE seed rows stay. Editor publish (ED-4) calls this after the successor exists. Finding [Q-3](#q-3-amendment-service-is-a-seed-dump). Sprint 25. | AMD-3, CNT-6, ED-4, GOV-6 |
| AMD-6 | Task | P1 | Done | S | amendment-service | Cite the amending law as data, not a ghost UUID. | New Flyway on `amendment_changes`: `amending_law_title TEXT`, `amending_law_citation TEXT`. Keep `amending_law_citation_id` nullable for a future citation-service (SRV-5); it is not required. Seed DE fills title + citation text for the 2022 transition. AMD-2/AMD-3 DTOs expose the two text fields. Do **not** scaffold `citation-service`. Finding [Q-11](#q-11-phantom-citation-service). Sprint 25. | AMD-3 |
| AMD-7 | Story | P0 | Done | L | amendment-service | Amendment identity + linear revisions (schema). | New Flyway `V7__amendment_revisions.sql`. Do not edit V2–V6. (1) `amendments` gains `constitution_id UUID NOT NULL` (opaque catalog id; backfill from `version_transitions.constitution_id` / seed), `kind TEXT NOT NULL DEFAULT 'legal_amendment'` CHECK (`legal_amendment`,`official_errata`), `status TEXT NOT NULL DEFAULT 'published'` CHECK (`draft`,`published`,`withdrawn`), `published_revision_id UUID` nullable. `version_transition_id` becomes nullable (historical laws need not point at snapshots). (2) New `amendment_revisions`: `id`, `amendment_id`, nullable `predecessor_revision_id` UNIQUE (linear; multiple NULL first-revisions forbidden per amendment via partial unique on `(amendment_id) WHERE predecessor_revision_id IS NULL`), `title`, `summary`, `enacted_on`, `effective_on`, `source_reference`, nullable `source_version_id`/`target_version_id`, `created_by` UUID, `created_at`. (3) `amendment_changes.amendment_id` replaced by `revision_id` (migrate existing change rows onto a new revision copied from the current amendment columns). (4) Seed DE: existing “Post-1949…” / 2022 rows become `kind=legal_amendment`, `status=published`, one published revision each; do not invent editorial hops. (5) CHECK: at most one `published_revision_id`, and it must belong to that amendment. `AmendmentRepository` tests (or Testcontainers in `AmendmentApiTest`) insert two revisions in a chain and reject a second child of the same predecessor. Sprint 29. | AMD-1, AMD-6, ARCH-7 |
| AMD-8 | Story | P0 | Done | L | amendment-service | Curated write API; stop persisting computed transitions. | `AmendmentController` write surface (Bearer, `WriteAccess.requireAmendmentWriter`; **publish/withdraw** additionally require `publisher`/`admin` — GOV-8). No in-place PATCH of a revision row. Routes: `POST /constitutions/{constitutionId}/amendments` body `{ kind, title, summary?, enactedOn?, effectiveOn?, sourceReference?, sourceVersionId?, targetVersionId?, changes[] }` → identity `status=draft` + first revision (editor/admin). `POST /amendments/{id}/revisions` same body → append at the tip only (409 if `predecessor` would branch). `POST /amendments/{id}/publish` sets `status=published` and `published_revision_id` to the tip (publisher/admin; 409 if already published at that tip). `POST /amendments/{id}/withdraw` sets `status=withdrawn` (publisher/admin; public lists omit it; staff still GET the chain). `POST /transitions` still inserts until **ED-6** (so editor publish stays green in Sprint 30); mark it deprecated in the controller KDoc. Keep AMD-5 matching code for AMD-10. Public DTOs expose `id`, `constitutionId`, `kind`, `status`, published revision fields, `changes`; never draft revision bodies. `AmendmentApiTest`: create → public list empty → publish → public list one row; second revision then publish replaces public title; withdraw hides it; viewer 403 on POST. Sprint 30. | AMD-7, GOV-6 |
| AMD-9 | Task | P0 | Done | M | amendment-service | Public vs staff reads. | `GET /constitutions/{id}/amendments` anonymous: `status=published` only, ordered by published revision `enacted_on` NULLS LAST then `created_at`. `GET /amendments/{id}` anonymous: 404 if draft or withdrawn. `GET /amendments?constitutionId=&articleNumber=` (AMD-4) same published-only filter. `GET /versions/{versionId}/amendments` returns published amendments whose published revision `target_version_id` (or source/target pair) matches; empty is valid (editorial successor, or no law recorded). Staff: `GET /amendments/{id}/revisions` requires editorial role; returns the linear chain oldest→newest (id, predecessor, actor, createdAt, full payload). 401/403 for anonymous/viewer. Existing AMD-2 tests updated: seed DE still returns the 2022 law; a draft revision is absent from public GET. Sprint 30. | AMD-8, AMD-2, AMD-4 |
| AMD-10 | Task | P1 | Done | M | amendment-service | Suggest change rows from two snapshots (no persist). | `POST /amendments/suggest` (Bearer, amendment writer) body `{ sourceVersionId, targetVersionId }` runs today’s `AmendmentDiff` (`AmendmentDiff.kt`) against content HTTP `includeBody=true`. Response is `{ changes: [{ articleNumber, changeType, note, nodeId, articleId }] }` — **no SQL insert**. 400 if ids equal; empty list if trees match. Reuse AMD-5 tests as suggest tests; delete or skip the persist-from-diff assertions. Sprint 33. | AMD-5, AMD-8, CNT-6 |
| AMD-11 | Story | P0 | Done | L | amendment-service | Change records: comment, documents, pins, `needs_review`. | New Flyway (do not edit V7). Drop `amendments.kind` (and its CHECK). Revisions: `comment TEXT` (migrate `summary`); `documents JSONB NOT NULL DEFAULT '[]'` of `{ "url"?, "fileId"?, "label"? }`; published pins `reviewed_source_tip_id` / `reviewed_target_tip_id` (opaque catalog snapshot ids, nullable for records with no quotes). `review_status TEXT NOT NULL DEFAULT 'ok'` CHECK (`ok`,`needs_review`). Partial UNIQUE on the **published** revision’s source pin and target pin (NULLs excluded): a snapshot is source of at most one published change record and target of at most one. `POST /constitutions/{id}/amendments/refresh-review-status` (Bearer, editor or internal): given a `legalVersionId`, set `needs_review` on published records whose pins ≠ live editorial tips of those legal identities; do not rewrite quotes or `published_revision_id`. Create/revision body: no `kind` (400 if sent); `comment` + `documents[]`. Public DTO: title, comment, documents, dates, changes; omit `kind`; anonymous may omit `reviewStatus`. Staff GET list `?reviewStatus=needs_review`. Seed DE: one published change record, comment from former citation/kind text, existing `source_reference` as one document URL if present. `AmendmentApiTest`: two published records cannot share a source pin; refresh-review-status flags a mismatch; public GET unchanged until a new revision is published. Sprint 36. | AMD-8, AMD-9, ARCH-8, CAT-6 |
...
| UI-ED-2 | Task | P0 | Done | M | gateway-web | Amendment workspace shell. | `/editor/amendments` and `/editor/amendments/[id]` live in the existing editor chrome (`PageMain` wide, `PageHeader`, editorial menu). Unauthenticated → `/login`. `viewer` → forbidden (`AdminForbidden` or shared editor forbidden). Server actions in `app/editor/amendments/actions.ts`. Stub pages may land in Sprint 32 with UI-43; this ID is the route+gate+client module (`lib/amendment-editor-api.ts`) so UI-43 does not invent a second session mechanism. Sprint 32. | UI-ED-1, IDN-8, AMD-8 |
...
| ED-3 | Task | P1 | Done | S | editor-service + gateway-web | List edit sessions by status and owner. | `GET /edit-sessions?status=&openedBy=&versionId=` returns `id`, `versionId`, `status`, `openedBy`, `openedAt`, `updatedAt`, `changedArticleCount`, newest first, max 100. Any editorial role may list; `openedBy=me` resolves from the session principal. Gateway `lib/editor-api.ts` gains `listSessions(filters)`. MockMvc test covers filter combinations and the 403 for `viewer`. Needed for the Review queue / Ready to publish / My sessions lists in `docs/design/role-link-trees.md`. Sprint 21. | ED-1, IDN-8 |
| ED-4 | Story | P0 | Done | L | editor-service | Publish creates a successor snapshot; never PATCH a published version. | `EditorService.publish`: after `approved` + step-up, (1) `POST` a new catalog draft version (label `{sourceLabel}-N` or a request field), (2) copy the source content tree onto that `versionId` (new node UUIDs; CNT-6 later adds `predecessor_id`), (3) apply `listLatestDrafts` **only** to the copy, (4) `POST /versions/{newId}/publish`, (5) call AMD-5 `POST /transitions` if that client exists (soft-fail + log until Sprint 25), (6) mark the session `published` with `{ sourceVersionId, newVersionId }`. Delete `EDITOR_PUBLISH_PUBLIC` and the in-place `contentClient.updateArticle` loop (`EditorService.kt` ~105–114). `EditorApiTest`: GET source article body equals pre-publish bytes; new version has the draft title/body. Gateway editor success alert names the new version. Finding [Q-2](#q-2-mutating-published-law). Sprint 23. | ED-2, IDN-8, ARCH-6, CAT-4, CNT-4 |
| ED-5 | Task | P1 | Done | S | editor-service | Session status is an enum, not a string. | Kotlin `enum class EditSessionStatus { OPEN, REVIEWING, APPROVED, PUBLISHED }` (DB values may stay lowercase with a converter). Flyway CHECK on `edit_sessions.status`. `requireStatus` compares enums. Illegal transition still 409. `EditorApiTest` unchanged in HTTP behaviour. Finding [Q-10](#q-10-stringly-typed-workflow). Sprint 24. | ED-1 |
| ED-6 | Story | P0 | Done | L | editor-service | Publish names the hop; never auto-records an amendment. | `POST /edit-sessions/{id}/publish` body (JSON or form fields already used by the gateway) requires `hopKind` in `legal_amendment` / `official_errata` / `editorial_correction`. (1) Session `versionId` must be the catalog chain tip (`GET /constitutions/{id}/versions?listing=all` / version DTO: no other published version has this id as `predecessorVersionId`); else 409 `not_tip`. (2) `createSuccessor` sends `predecessorVersionId = session.versionId` and that `hopKind` to catalog (CAT-5). (3) **Stop** `amendmentClient.recordTransition` in `EditorService.publish` and `RestAmendmentClient` persist; `POST /transitions` then returns 410 `use_amendment_write_api`. Delete `AMENDMENT_RECORDED` outbox when hop is `editorial_correction`. (4) `legal_amendment` and `official_errata` require `amendmentId` of a `draft` or `published` amendment for that constitution; after the successor exists, `POST /amendments/{id}/revisions` (or a dedicated `link-target` if the tip revision already has empty target) setting `targetVersionId` (and `sourceVersionId` if unset) then `POST /amendments/{id}/publish` if still draft. Missing/wrong amendment → 400, successor must not be left published without rolling forward — prefer validating amendmentId **before** catalog publish. (5) `editorial_correction` forbids `amendmentId`. `EditorApiTest`: editorial publish leaves amendment public list unchanged; legal publish with amendmentId shows that law’s target version; session on a non-tip version → 409. Update ADR 0003 payload notes if `amendment.recorded` now carries `amendmentId`. Sprint 31. | ED-4, CAT-5, AMD-8, ARCH-7 |
| ED-7 | Story | P0 | Ready | L | editor-service | Publish from a legal version’s editorial tip. | `POST /edit-sessions/{id}/publish` accepts `hopKind` `legal` \| `editorial_correction` only (`legal_amendment`/`official_errata` → 400). Session `versionId` must be the **editorial tip of its legal identity** (CAT-6) else 409 `not_editorial_tip`. Which hopKind is used is determined by how the session was opened (UI-52): transcription vs next legal change — not a two-choice dialog. (1) `editorial_correction`: required Wikipedia `comment`; forbid `amendmentId`; catalog hop `editorial_correction` with editorial predecessor = session version. (2) `legal`: required change-record payload or `amendmentId`; session must also be the editorial tip of the current **legal** tip else 409 `not_legal_tip`; catalog hop `legal`; after the successor exists, pin the published change-record revision to (old law tip, new law tip) and publish the record if still draft. Refuse legal publish if the instrument lacks title/comment/documents. (3) After any `editorial_correction` publish, call AMD-11 `refresh-review-status` for that legal identity immediately. `EditorApiTest`: typo-fix of 1949 while a later legal version exists succeeds and does not attach a change record; legal publish from 1949 when a later legal tip exists → 409; editorial publish sets `needs_review` on a change record still pinned to the previous 1949 snapshot. UI: [`#correct-1949`](docs/design/prototype/editor-constitution.html#correct-1949) vs [`#legal`](docs/design/prototype/editor-constitution.html#legal). Sprint 37. | ED-6, CAT-6, AMD-11, ARCH-8 |
...
| IDN-10 | Story | P1 | Done | M | identity-service + gateway-web | MFA for privileged users. | TOTP or WebAuthn enrollment, recovery, and revocation are available; MFA is mandatory for admin/publisher; publish and role changes require recent step-up authentication; recovery is audited. | IDN-5, IDN-8, IDN-9 |
...
| IDN-14 | Story | P0 | Done | M | identity-service + ingestion + editor + search | Scoped machine authentication for service-to-service writes. | Identity issues a rotatable Bearer for automation (`POST /service-tokens` admin-only, hashed at rest, scope list). Scopes at least: `catalog:write`, `content:write`, `search:reindex`, `audit:append`, `ingestion:import`. Ingestion write routes and search `POST /reindex` accept this credential instead of a human password. Tokens are not seeded in production. Editor-service sends `EDITOR_SERVICE_TOKEN` for content PATCH, search reindex, and audit append (publishers have no `content:write`). Finding [Q-1](#q-1-public-write-apis). Sprint 22. | IDN-8, ING-1, GOV-6 |
| IDN-15 | Task | P0 | Done | S | gateway-web | Invite and reset tokens never appear in the URL. | `inviteUserAction` / `issueResetAction` in `apps/gateway-web/app/account/actions.ts` must not `redirect(/admin/users?invited=…)` or `?reset=…`. Show the one-time token in the POST response (server-rendered alert) or an HttpOnly flash cookie consumed once. Remove `searchParams.invited` / `searchParams.reset` from `app/admin/users/page.tsx`. Tokens must not appear in Referer, access logs, or browser history. Finding [Q-14](#q-14-invite-and-reset-tokens-in-the-query-string). Sprint 22. | IDN-5, IDN-7 |
...
| SRC-3 | Story | P1 | Done | M | gateway-web | In-text cross-reference links. | Pure `linkifyReferences(text, { code, versionId, articlesByNumber })` in `lib/crossrefs.ts` returns React nodes; patterns are driven by the outline label (`kindByCode`), e.g. DE `Art(ikel\|\.)\s*(\d+[a-z]?)`, EN `Article\s+(\d+)`. Only numbers present in `articlesByNumber` (from `listAllArticles`) become links to the same version's article. Unit tests with at least five strings including a non-existent number. Applied inside `ConstitutionText`; print view unchanged. Sprint 16. | UI-2, UI-11 |
| SRC-4 | Story | P2 | Ideas | L | search-service | Global term index. | Later. | DB-4 |
| SRC-6 | Story | P1 | Done | M | search-service + gateway-web | Search paging and highlighted snippets. | `GET /search` accepts `offset` (default 0) and returns `{ hits, total, limit, offset }`; `snippet` uses `ts_headline` with `StartSel=<mark>, StopSel=</mark>`. Gateway `lib/api.ts`, `SearchHit`, and the contracts fixture follow; snippet is rendered by splitting on the `<mark>` tag only (no `dangerouslySetInnerHTML`); Previous/Next links preserve all query params. `SearchApiTest` covers offset/total and a highlighted snippet. Sprint 17. | SRC-1, SRC-5 |
| SRC-7 | Task | P1 | Done | S | search-service + gateway-web | Country names in facets. | New Flyway adds `country_name` to `search_documents`, filled at reindex from catalog HTTP; `/search/facets` countries include `countryName`; `/search` renders `Germany (12)` instead of `DE (12)`. Sprint 17. | SRC-2, SRC-5 |
| SRC-8 | Task | P1 | Done | M | search-service | Language-aware full text (German stemming). | Stop using only `to_tsvector('simple', …)` in `V2__search_index.sql`. New Flyway: store `language_code` on `search_documents` (from catalog at reindex). Generated `tsv` uses `german` when `language_code` is `de`, else `simple`. Keep a `simple` tsvector (or the same column plus an expression) on `article_number` so `1`, `1a`, and `Art. 1` still match. `SearchApiTest` asserts a German inflected query hits the DE seed body. Finding [Q-9](#q-9-search-uses-simple-tsvector). Sprint 26. | SRC-1, SRC-5 |
...
| GOV-6 | Story | P0 | Done | L | catalog + content + audit + search + ingestion | Mutating APIs require a valid identity Bearer. | POST/PUT/PATCH/DELETE on catalog (`createCountry`, `createConstitution`, `createVersion`, `publishVersion`, `putOutline`), content (`replaceArticles`, `patchArticle`, `patchNode`, `restructure`), audit (`POST /events`), search (`POST /reindex`), and ingestion import jobs return **401** without `Authorization: Bearer` that identity `/me` accepts, and **403** when the caller lacks the capability (`editor`/`admin` for catalog+content writes; `publisher`/`admin` for reindex; `editor`/`admin`/ingestion job actor for `audit:append`; `admin` for import). GET list/get/search stay anonymous. Do not wait for PLAT-5 — copy editor’s identity client if needed; PLAT-5 deletes the copies. MockMvc: one 401 and one authorized 2xx per mutating controller above. Existing GET tests stay green without Authorization. Bearer (not cookie) on these APIs is the CSRF control: a foreign site cannot POST `/api/content*` using `ca_session` alone. Finding [Q-1](#q-1-public-write-apis). Sprint 22. | IDN-2, IDN-8 |
| GOV-7 | Task | P2 | Ideas | S | identity-service + audit-service | Fold audit-service only if it stays a single append endpoint. | After GOV-6, if audit-service is still ~one POST and one GET, consider moving `audit_events` into identity or editor rather than paying an eighth process. Not a Sprint 22–27 item; do not do this while GOV-6 is open. Finding [Q-6](#q-6-eight-cloned-microservices). | GOV-6, PLAT-5 |
| GOV-8 | Task | P0 | Done | S | amendment-service | Amendment publish/withdraw is publisher-only. | `POST /amendments/{id}/publish` and `/withdraw` return 403 for `editor` and `reviewer`; 2xx for `publisher`/`admin`. Draft create + `POST /revisions` remain editor/admin (and publisher if they also hold editor — local seed does). MockMvc in `AmendmentApiTest`. Align `docs/editorial-roles.md`. Sprint 30. | AMD-8, IDN-8 |
| SEC-1 | Story | P0 | Done | L | gateway-web | Upgrade Next.js off the EOL 14.2.5 line. | Move `next` and `eslint-config-next` to a supported 15.5.x (or current LTS) release that includes GHSA-89xv-2m56-2m9x. Keep React 18 if 15 still supports it. Re-run `npm test`, `lint`, `build`, and Playwright; refresh visual snapshots if the App Router markup shifts. Finding [S-F-1](docs/security-review-2026-09-08.md). Sprint 28. | UI-ED-1 |
| SEC-2 | Task | P1 | Done | M | infra/caddy + gateway-web | Content-Security-Policy with nonces. | Caddy (or `next.config.js` `headers()`) ships a CSP including `frame-ancestors 'none'`. Next 14/15 inline scripts need nonces or `'unsafe-inline'` documented as temporary. Exempt `/api/docs/*` if Swagger stays. Finding S-F-5. Sprint 28. | SEC-1 |
| SEC-3 | Task | P1 | Done | S | infra/caddy | Restrict OpenAPI/Swagger in non-local profiles. | `/api/docs/*` is not reachable on production (Caddy env import, `basicauth`, or `forward_auth` to identity `/me`). Local-stack may keep it. Finding S-F-15. Sprint 28. | OPS-9 |
| SEC-4 | Story | P0 | Done | M | audit-service | Authenticate audit event reads. | `GET /events` returns 401 without a Bearer that identity `/me` accepts, and 403 unless `admin` or a new `audit:read` scope. MockMvc covers 401 and authorized 200. gateway-web does not call this yet. Finding S-B-1. Sprint 28. | GOV-6 |
| SEC-5 | Story | P0 | Done | M | catalog-service + identity-service | Catalog publish is publisher-only. | `POST /versions/{id}/publish` requires `publisher`/`admin` or a `catalog:publish` scope (add to `ServiceTokenService.ALLOWED_SCOPES` and seed/editor tokens). Editors get 403. Finding S-B-3. Sprint 28. | GOV-6, IDN-8, IDN-14 |
| SEC-6 | Task | P1 | Done | M | all services | HTTP client timeouts. | Every `RestClient` uses connect ≤2s and read ≤5s (`spring.http.client.*` or a shared factory). Identity’s audit call must not stall login unbounded. Finding S-B-7. Sprint 28. | PLAT-5 |
| SEC-7 | Task | P1 | Done | M | identity-service | Service token expiry and rotation. | New Flyway `expires_at`; lookup rejects expired rows; admin UI/API can rotate. Finding S-B-16. Sprint 28. | IDN-14 |
| SEC-8 | Task | P2 | Ideas | M | audit-service + infra | Least-privilege DB role for audit. | Runtime role has `INSERT, SELECT` only on `audit_events`; Flyway uses owner. Finding S-B-18. Not a Sprint 28 item. | GOV-4 |
| SEC-9 | Task | P2 | Done | S | all services | Allow-list `X-Correlation-Id`. | Accept `^[A-Za-z0-9._-]{1,64}$` or mint a UUID. Prefer landing this in PLAT-5 instead of editing eight copies. Finding S-B-15. Sprint 26. | PLAT-5 |
| SEC-10 | Task | P2 | Done | S | .github | Pin Actions to commit SHAs. | `actions/checkout`, `setup-java`, `setup-node`, `setup-gradle`, `upload-artifact` use full SHAs; Dependabot `github-actions` keeps them current. Finding S-F-8. Sprint 28. | CI-1 |
| SEC-11 | Task | P1 | Done | M | content + ingestion + editor | Cap JSON body size and nesting. | Reject oversized `PUT /versions/{id}/articles`, `POST /import-jobs`, and editor saves (Content-Length filter or Jackson `StreamReadConstraints`). Finding S-B-14. Sprint 28. | ING-1, CNT-3 |
...
| PLAT-5 | Task | P1 | Done | M | repo / gradle | Shared Kotlin platform module; delete the eight copies. | New included Gradle project (e.g. `services/platform`) containing one `CorrelationIdFilter`, one `NotFoundException` + problem-JSON advice, and the identity Bearer filter introduced in GOV-6. Each of the eight services depends on it. `rg "class CorrelationIdFilter"` and `rg "class NotFoundException"` have a single Kotlin body. Update `services/AGENTS.md`: stop saying “Copy a sibling service” for filters/exceptions; depend on platform. Dockerfiles may stay copied. Finding [Q-6](#q-6-eight-cloned-microservices). Sprint 26. | PLAT-4, GOV-6 |
| PLAT-6 | Task | P1 | Done | M | editor-service | Outbox for search reindex (and audit if still HTTP). | After ED-4 content copy + catalog publish commit, write `outbox_events` in the **editor** database (`event_name`, `payload JSONB`, `created_at`, `published_at`). A scheduler publishes `search.reindex-requested` (SRV-4). `RestSearchIndexClient` failures retry; they do **not** swallow inside `EditorService.publish` with only a log. Session may be `published` after the snapshot commit, but `searchIndexStatus` (`pending`/`ready`/`failed`) is visible on the preview DTO. No Kafka. Finding [Q-8](#q-8-swallowed-reindex--no-outbox). Sprint 27. | ED-4, SRV-4 |
...
| CI-6 | Task | P1 | Done | M | .github | One real publish journey against Compose (not the Playwright mock API). | New CI job (or an addition to `e2e`) boots identity + catalog + content (+ editor if needed) with Testcontainers or `docker compose`, creates a draft version, publishes via ED-4, then `GET` the **source** version body and asserts it equals the pre-publish bytes, and `GET` the new version for the draft text. Playwright `e2e/mock-api.mjs` is unchanged for UI tests. Finding [Q-13](#q-13-e2e-against-a-mock-api). Sprint 27. | ED-4, CI-2 |
...
| QLT-5 | Task | P1 | Done | M | editor | Feature flag for publish. | Editor publish rewrites public content (not only audit + session status). Sprint 9, after publish authorization is explicit. **Superseded for the happy path by ED-4 / ARCH-6** (do not re-open; delete the flag in QLT-9). | ED-2, IDN-8 |
...
| QLT-9 | Task | P1 | Done | S | editor-service | Remove QLT-5 in-place publish leftovers. | After ED-4: `EDITOR_PUBLISH_PUBLIC` gone from `env/*.env.example`, `docker-compose.yml`, and `EditorPublishProperties`. `rg updateArticle` in editor-service has no publish loop. Finding [Q-2](#q-2-mutating-published-law). Sprint 23 (same change as ED-4 if already done there — close this ID together). | ED-4, QLT-5 |

## Quality review (2026-09-06)

Seminar-style code-quality review of this repo (advanced-student bar). Full write-up: [seminar-code-quality-grade.canvas.tsx](/Users/julianhoelz/.cursor/projects/Users-julianhoelz-Documents-Verfassung/canvases/seminar-code-quality-grade.canvas.tsx) (open beside chat in Cursor).

Grade recorded there: **2.3 / B−**. Sprints 22–27 below are the revision plan. Do not re-open Done IDs; QLT-5 in-place publish is superseded by ED-4.

| ID | Finding | Severity | Sprint | Stories |
| --- | --- | --- | --- | --- |
| <a id="q-1-public-write-apis"></a>Q-1 | Public write APIs: Caddy proxies `/api/catalog*`, `/api/content*`, `/api/audit*`, `/api/search*` with no auth; controllers accept unauthenticated PUT/PATCH/POST (replace articles, publish version, append audit, reindex). | Fixed | 22 | GOV-6, OPS-9, IDN-14 |
| <a id="q-2-mutating-published-law"></a>Q-2 | Publish patches existing public articles in place (`EditorService.publish` + `EDITOR_PUBLISH_PUBLIC`). Violates ARCH-2 / CNT-1 “snapshot rows immutable.” | Fixed | 23 | ARCH-6, CAT-4, CNT-4, ED-4, QLT-9 |
| <a id="q-3-amendment-service-is-a-seed-dump"></a>Q-3 | Amendment-service is a seed dump: `AmendmentController` → repository only; change rows are SQL fixtures; real LCS lives in `gateway-web/lib/text-diff.ts`. | Fixed | 25 | AMD-5 |
| <a id="q-4-dual-write-articles--content_nodes"></a>Q-4 | Dual write: `ArticleRepository.replaceForVersion` INSERT/DELETE both `articles` and `content_nodes`. Tree-is-source-of-truth is a migration comment. | Fixed | 24 | CNT-5 |
| <a id="q-5-unstable-article-identity"></a>Q-5 | `replaceForVersion` always `UUID.randomUUID()`. Amendment `node_id` and compare-by-id break across re-import / copy. | Fixed | 24 | CNT-6 |
| <a id="q-6-eight-cloned-microservices"></a>Q-6 | Eight copies of `CorrelationIdFilter`, Dockerfile, logback, `NotFoundException`, SmokeTest. House style “copy a sibling.” amendment/audit do not earn a process until they have domain logic / auth. | Fixed | 26 (fold later) | PLAT-5, GOV-7 |
| <a id="q-7-english-sentence-splitter-on-german-law"></a>Q-7 | `text-diff.ts` splits on `[.!?]`. GG units are Art. / Abs. / Satz. | Fixed | 26 | UI-40 |
| <a id="q-8-swallowed-reindex--no-outbox"></a>Q-8 | Publish reindex is best-effort HTTP inside `@Transactional`; failures logged and swallowed. No outbox (ADR 0001 deferred it). | Fixed | 27 | SRV-4, PLAT-6 |
| <a id="q-9-search-uses-simple-tsvector"></a>Q-9 | `to_tsvector('simple', …)` — no German stemming, weak article-number search. | Fixed | 26 | SRC-8 |
| <a id="q-10-stringly-typed-workflow"></a>Q-10 | Edit session status is `'open'` / `'reviewing'` / … strings. | Fixed | 24 | ED-5 |
| <a id="q-11-phantom-citation-service"></a>Q-11 | `amending_law_citation_id` is an opaque UUID to a service that does not exist (SRV-5). | Fixed | 25 | AMD-6 |
| <a id="q-12-axe-contrast-disabled"></a>Q-12 | `e2e/a11y.spec.ts` disables `color-contrast` globally. | Fixed | 26 | UI-41 |
| <a id="q-13-e2e-against-a-mock-api"></a>Q-13 | Playwright points `*_API_URL` at `e2e/mock-api.mjs`. Proves the UI, not catalog/content agreement. | Fixed | 27 | CI-6 |
| <a id="q-14-invite-and-reset-tokens-in-the-query-string"></a>Q-14 | Admin invite/reset tokens in `?invited=` / `?reset=` (`account/actions.ts` redirect). Leak via Referer/history/logs. | Fixed | 22 | IDN-15 |

## Security review (2026-09-08)

Scan of gateway-web, Caddy/Compose, and all eight Kotlin services. Full write-up: [`docs/security-review-2026-09-08.md`](docs/security-review-2026-09-08.md). Small items were fixed in the same change; the rows below are the leftovers.

| ID | Finding | Severity | Status | Sprint | Stories |
| --- | --- | --- | --- | --- | --- |
| S-F-1 | Next.js 14.2.5 is EOL (Server Actions SSRF GHSA-89xv-2m56-2m9x). Caddy now pins `Host`; still need a supported Next line. | High | Fixed | 28 | SEC-1 |
| S-F-5 | No Content-Security-Policy (other browser headers landed). | Medium | Fixed | 28 | SEC-2 |
| S-F-15 | `/api/docs/*` Swagger/OpenAPI is public, including identity and editor. | Low | Fixed | 28 | SEC-3 |
| S-B-1 | Unauthenticated `GET /events` leaks login IPs, emails, draft bodies. | High | Fixed | 28 | SEC-4 |
| S-B-3 | Catalog `POST /versions/{id}/publish` allows any editor / `catalog:write` (no publisher + step-up). | Medium | Fixed | 28 | SEC-5 |
| S-B-7 | Inter-service `RestClient`s have no connect/read timeouts. | Medium | Fixed | 28 | SEC-6 |
| S-B-16 | Service tokens never expire. | Info | Fixed | 28 | SEC-7 |
| S-B-18 | Audit append-only rules omit `TRUNCATE` and the table owner can `DROP RULE`. | Info | Open | Ideas | SEC-8 |
| S-B-15 | `X-Correlation-Id` echoed without an allow-list (eight copies). | Low | Done | 26 | SEC-9, PLAT-5 |
| S-F-8 | GitHub Actions pinned by mutable `@v4` tag, not SHA. | Low | Fixed | 28 | SEC-10 |
| S-B-14 | No JSON body size / nesting cap on import, article replace, editor save. | Low | Fixed | 28 | SEC-11 |

## Suggested Sprint Breakdown

Pull **only Ready P0 tasks** listed under each sprint. Do not re-open Done IDs.

Recommended order after Sprint 15: **22 → 23 → 19 → 20 → 21 → 16 → 17 → 18 → 28 → 24 → 25 → 26 → 27**. After that: **29 → 30 → 31 → 32 → 33** (legal amendments vs editorial snapshots).

P0 quality (lock writes, then immutable publish) lands **before** the redesign so UI-38 does not restyle an in-place publish that ED-4 removes. Reader/search/discovery (16–18) still wait for the new shell (19–21). **Sprint 28** is the leftover High/Medium items from the 2026-09-08 security scan, after discovery (18) and before the content-model rewrite (24). Content-model, amendment, platform, and search-language work follow. Findings: [Quality review (2026-09-06)](#quality-review-2026-09-06), [Security review (2026-09-08)](#security-review-2026-09-08).
...
Done: IDN-8, QLT-5.
`reviewer` and `publisher` roles are separate from `editor`; editor-service returns 403 when the actor lacks the command capability. Approving a review is `POST .../approval`; publish requires `approved`. In-place public rewrite (QLT-5) was superseded by ED-4 in Sprint 23. Roles: `docs/editorial-roles.md`.
...
### Sprint 14: Editor quality and confidence — closed
Done: **IDN-10**, **UI-10**, **UI-14**.
Privileged accounts enroll TOTP, recover, and revoke MFA; publish and role changes require a recent step-up. Compare renders node- and sentence-level diffs with AMD-3 records kept. Playwright journeys, axe, and visual snapshots run in CI against contract fixtures.

### Sprint 15: Content acquisition — import from the browser — closed
Done: **ING-6**, **ING-3**, **UI-21**, **ING-7**.
Import accepts a content tree and an outline, forwards provenance to catalog sources, and an admin page at `/admin/import` uploads JSON, polls the job, and shows `import_errors`. A second country ships as a JSON fixture.

### Sprint 16: Reader navigation — through the text and through time — closed
Done: **UI-22**, **AMD-4**, **UI-23**, **SRC-3**.
Article previous/next/`Table of contents` via `neighborsOf` + `ArticleNav`. `GET /amendments?constitutionId=&articleNumber=` (V5 `constitution_id`; `article_number` already existed). History page `/countries/[code]/articles/[articleNumber]` with compare-with-previous and AMD-4 rows. In-text `Article N` / `Art. N` links in `ConstitutionText`. Owners: amendment-service + gateway-web.

### Sprint 17: Search everywhere — closed
Done: **UI-24**, **SRC-6**, **SRC-7**, **UI-25**.
...
### Sprint 18: Discovery, sharing, and correct language — closed
Done: **UI-15**, **UI-26**, **UI-27**, **UI-28**.
...
### Sprint 19: Redesign I — design system and shell — closed
Done: **UI-29**, **UI-30**, **UI-31**, **UI-32**.
Tokens from `docs/design/tokens.css` are in `globals.css` with a temporary alias block (removed in UI-39). Primitives: `Badge`, `Chip`, `PageHeader`, `SiteFooter`, `Pager`, `DataList`. Sticky top bar with desktop primary nav, pill search, phone search row, and a role-aware `MenuDisclosure` from `menuSections(user)`. `/about` and `/admin` index routes. Owner: gateway-web.

### Sprint 20: Redesign II — public reading pages — closed
Done: **UI-33**, **UI-34**, **UI-35**, **UI-36**.
Concept: `ui-concept.md` §4.1–4.4, 4.6–4.7, 4.10. Home hero + country cards, country chip rows, version reader with TOC / Contents / Segmented detail / paragraph gutter, article page, timeline, search with filter sidebar, and single-card account/login/invite/reset. Owner: gateway-web. Search snippet `<mark>` (SRC-6), country names in facets (SRC-7), and search paging stay Sprint 17. Alias CSS remains until UI-39.

### Sprint 21: Redesign III — compare, editor, admin, verification — closed
Done: **UI-37**, **ED-3**, **UI-38**, **UI-39**.
Concept: `ui-concept.md` §4.5, 4.8–4.9. Sticky compare bar, phone tabs, hop details, two-column cards; editor workspace with workflow steps, three panels, sticky action bar, and ED-3 session lists; admin Users/Outlines/Import on `PageHeader` + `DataList`. Token alias CSS and old classes removed. Visual snapshots re-baselined at 390 / 820 / 1440 in Linux Chromium (`mcr.microsoft.com/playwright:v1.49.1-jammy`). Axe covers home, search, login, about, version, article, compare, editor, and admin users. Lighthouse ≥ 95 was not scored. Owners: gateway-web + editor-service (ED-3).

### Sprint 22: Lock the public write surface — closed
Done: **GOV-6**, **OPS-9**, **IDN-14**, **IDN-15**. Findings [Q-1](#q-1-public-write-apis), [Q-14](#q-14-invite-and-reset-tokens-in-the-query-string).

Anonymous GET on catalog/content/amendment/search stays. Every mutating method on catalog, content, audit, search, and ingestion returns 401 without a Bearer that identity `/me` accepts, and 403 without the right role. Caddy additionally rejects those methods when `Authorization` is missing. Ingestion/search automation gets rotatable machine tokens (IDN-14). Invite and reset tokens leave the query string. Editor uses `EDITOR_SERVICE_TOKEN` for content, search, and audit because publishers lack `content:write`.

Owners: catalog, content, audit, search, ingestion, identity, Caddy, gateway-web (`admin/users` only for IDN-15). Land **GOV-6 first**, then IDN-14 (machine tokens). Do not extract a platform module yet (PLAT-5). Do not start SRV-7.

### Sprint 23: Immutable published snapshots — closed
Done: **ARCH-6**, **CAT-4**, **CNT-4**, **ED-4**, **QLT-9**. Finding [Q-2](#q-2-mutating-published-law).

ADR 0002: published `versionId` never changes bytes. Catalog `GET /versions/{id}` exposes `publicationStatus`. Content mutating APIs 409 when that status is `published`. Editor publish copies the tree onto a **new** catalog version, applies drafts there, publishes the successor, and deletes `EDITOR_PUBLISH_PUBLIC` / the in-place `updateArticle` loop. After catalog publish the session is `published`; reindex/audit failures are logged and do not mint another successor.

Owner: docs + catalog (S) + content (M) + editor (L). One L only (ED-4).

### Sprint 24: One content write model — closed
Done: **CNT-5**, **CNT-6**, **ED-5**. Findings [Q-4](#q-4-dual-write-articles--content_nodes), [Q-5](#q-5-unstable-article-identity), [Q-10](#q-10-stringly-typed-workflow).

`articles` is a view over root `content_nodes`. Optional client node ids; `predecessor_id` on copy. Edit session status is `EditSessionStatus` plus a CHECK constraint.

Owner: content-service + a small editor follow-up (predecessor on copy, status enum).

### Sprint 25: Amendment-service earns its process — closed
Done: **AMD-5**, **AMD-6**. Findings [Q-3](#q-3-amendment-service-is-a-seed-dump), [Q-11](#q-11-phantom-citation-service).

`AmendmentService` computes `added`/`changed`/`removed` from two content trees (`POST /transitions`). Controller no longer talks to the repository. Amending-law title + citation text on the change row; do not build citation-service. Editor publish calls the new endpoint (ED-4 already soft-fails until this sprint).

Owner: amendment-service (L) + DTO fields only. ING-4 stays Ideas.

### Sprint 26: Shared platform, German search, honest diffs — closed
Done: **PLAT-5**, **SEC-9**, **SRC-8**, **UI-40**, **UI-41**. Findings [Q-6](#q-6-eight-cloned-microservices), [Q-7](#q-7-english-sentence-splitter-on-german-law), [Q-9](#q-9-search-uses-simple-tsvector), [Q-12](#q-12-axe-contrast-disabled), S-B-15.

One Gradle `platform` module; eight `CorrelationIdFilter` / `NotFoundException` copies deleted. The shared filter allow-lists `X-Correlation-Id` (SEC-9). Search `tsv` uses `german` for `de`. Compare diffs word-diff leaves (no `[.!?]` split on Art./Abs.). Axe color-contrast enabled in `e2e/a11y.spec.ts` (Playwright not re-run on 8GB hosts).

Owners: repo/gradle, search-service, gateway-web.

### Sprint 27: Outbox and a real publish journey — closed
Done: **SRV-4**, **PLAT-6**, **CI-6**. Findings [Q-8](#q-8-swallowed-reindex--no-outbox), [Q-13](#q-13-e2e-against-a-mock-api).

`docs/adr/0003-domain-events.md` names `version.published` / `amendment.recorded` / `search.reindex-requested` (no broker). Editor `outbox_events` retries search reindex; preview `searchIndexStatus` is `pending`/`ready`/`failed`. CI job `publish-journey` boots identity+catalog+content+editor and asserts the source version is unchanged after ED-4 publish. Playwright mock API stays for UI e2e. Local Compose journey was not run on 8GB hosts.

Owners: docs + editor-service (SRV-4, PLAT-6), `.github` + `scripts/publish_journey.py` (CI-6).

### Sprint 28: Security review leftovers — closed
Done: **SEC-1**, **SEC-2**, **SEC-3**, **SEC-4**, **SEC-5**, **SEC-6**, **SEC-7**, **SEC-10**, **SEC-11**. Findings [Security review (2026-09-08)](#security-review-2026-09-08).

Next.js 15.5.24 (React 18), CSP nonces in gateway `middleware.ts`, Swagger 404 unless `CADDY_EXPOSE_API_DOCS=true`, audit `GET /events` requires admin or `audit:read`, catalog publish is publisher/`catalog:publish` only, RestClient connect 2s / read 5s, service-token `expires_at` plus rotate, Actions SHA-pinned, Jackson JSON nesting and size caps. **SEC-9** stays Sprint 26; **SEC-8** stays Ideas. Identity OpenAPI `NoSuchMethodError` (S-B-19) is unchanged. Local `next build` / Playwright were not re-run on 8GB hosts.

Owners: gateway-web (SEC-1, SEC-2), audit-service (SEC-4), catalog + identity (SEC-5), all services (SEC-6), identity (SEC-7), Caddy (SEC-3), GitHub Actions (SEC-10), content + ingestion + editor (SEC-11).

### Sprint 29: Hop kinds and amendment revisions (schema) — closed
Done: **ARCH-7**, **CAT-5**, **AMD-7**.

Write ADR 0004. Catalog `V9__version_chain.sql`: linear `predecessor_version_id`, `hop_kind`, `listing`; public version list hides `editorial_correction`; country latest = chain tip. Amendment `V7__amendment_revisions.sql`: stable amendment id, linear `amendment_revisions`, changes hang off a revision; `version_transition_id` nullable; seed DE laws stay published `legal_amendment`.

Owners: docs (ARCH-7), catalog-service (CAT-5), amendment-service (AMD-7). One L (AMD-7).

### Sprint 30: Curated amendment API — closed
Done: **AMD-8**, **AMD-9**, **GOV-8**.

Write path is create + append revision + publish/withdraw. `POST /transitions` stays until ED-6. Public GET is published-only; staff GET `/revisions` is the covert amendment history. Publish/withdraw publisher-only.

Owner: amendment-service. One L (AMD-8).

### Sprint 31: Editor publish and public surfaces — closed
Done: **ED-6**, **UI-42**, **UI-46**, **UI-49**, **UI-50**.

Editor publish requires hop kind, only from the chain tip, and never auto-records an amendment. Legal/errata hops must link an `amendmentId`. Public timeline and compare hops are curated laws. Public version chips omit editorial hops; readers of “latest” still get the tip text.

Owners: editor-service (ED-6), gateway-web (UI-*). One L (ED-6). UI-43 is not in this sprint; the publish `<Select>` uses the AMD-9 staff list (seed DE).

### Sprint 32: Amendment list editor — closed
Done: **UI-ED-2**, **UI-43**, **UI-48**.

Signed-in `/editor/amendments` workspace: constitution list, Add amending law, form with change table, Save draft / Publish law / Withdraw. Editorial nav link. Substantial editor IA change is intentional.

Owner: gateway-web. One L (UI-43).

### Sprint 33: Covert histories, suggest-from-diff, journeys — closed
Done: **AMD-10**, **UI-44**, **UI-45**, **UI-47**, **UI-51**.

`POST /amendments/suggest` diffs two content trees and returns change rows with no SQL insert. Staff revision panel restores as a new tip; “This is public” follows `publishedRevisionId`. Staff snapshot history lists `listing=all` hops. Playwright journeys cover recording a law vs an `editorial_correction` hop (not re-run on 8GB hosts).

Owners: amendment-service (AMD-10), gateway-web. No L.

**Design for Sprints 34–39:** [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) (**Sprint UI reference** = configured `prototype/editor-constitution.html` hashes) · Cursor canvas [legal vs editorial chain](/Users/julianhoelz/.cursor/projects/Users-julianhoelz-Documents-Verfassung/canvases/legal-vs-editorial-chain.canvas.tsx)

### Sprint 34: Two-axis ADR — closed
Done: **ARCH-8**.

ADR 0005 Accepted. Two axes (legal versions × editorial revisions). Catalog hop kinds `initial` / `legal` / `editorial_correction` only. Change records are comment + documents, not an errata enum. Pins, `needs_review`, and the at-most-one forward/backward association per law snapshot. Canvas and [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) § Sprint UI reference linked from the ADR.

Owner: docs. No L.

### Sprint 35: Catalog legal vs editorial chains — closed
Done: **CAT-6**.

Per-legal-version editorial chain. Public chips = legal identities with `currentVersionId` = editorial tip. Split `409` (`not_legal_tip` vs `not_editorial_tip`). One L.

Owner: catalog-service.

### Sprint 36: Change records — closed
Done: **AMD-11**.

Drop kind enum. Comment + documents on revisions. Published pins + partial unique associations. `needs_review` + `refresh-review-status`. One L.

Owner: amendment-service.

### Sprint 37: Publish from any law column
Ready: **ED-7**, **UI-52**.

How you open the session picks the job (P2 vs P3). Transcription publish from that legal version’s editorial tip (Wikipedia comment, no change-record id). **Record the next legal change** is a session on the legal tip with the instrument in the sidebar; legal publish mints the new chip and the change record together. No two-choice Publish dialog. One L (ED-7).

Mocks: [`editor-constitution.html#legal`](docs/design/prototype/editor-constitution.html#legal) (P2), [`#correct-1949`](docs/design/prototype/editor-constitution.html#correct-1949) (P3). Table: [`two-axis-versions.md`](docs/design/two-axis-versions.md) § Sprint UI reference.

Owners: editor-service (ED-7), gateway-web (UI-52).

### Sprint 38: Public legal edition
Ready: **UI-53**, **UI-54**, **UI-58**.

Chips, timeline (comment + document links, no errata badge), and compare follow legal identities and published change records.

No two-axis **public** mock yet. Do not copy `prototype/home.html`, `version.html`, or `compare.html` for versioning (Sprint 19–21 shell; still 1949/2022 and “amending laws”). Staff visual language for public-version vs legal-change cards: [`#timeline`](docs/design/prototype/editor-constitution.html#timeline).

Owner: gateway-web. No L.

### Sprint 39: Staff history and quote review
Ready: **UI-55**, **UI-56**, **UI-57**, **UI-59**.

Grouped snapshot history, Legal changes form (comment + documents), needs-review inbox + nav badge, Playwright journeys for correcting an old law.

Mocks: [`#timeline`](docs/design/prototype/editor-constitution.html#timeline) (UI-55), [`#hop-1956`](docs/design/prototype/editor-constitution.html#hop-1956) / [`#hop-2022`](docs/design/prototype/editor-constitution.html#hop-2022) / [`#hop-1994`](docs/design/prototype/editor-constitution.html#hop-1994) (UI-56), change-record sidebar on [`#legal`](docs/design/prototype/editor-constitution.html#legal) (UI-57). Table: [`two-axis-versions.md`](docs/design/two-axis-versions.md) § Sprint UI reference.

Owner: gateway-web. No L.

### Later / Ideas
SRV-5, **SRV-7 (MCP, postponed)**, **GW-2**, IDN-11, ING-4–5, SRC-4, PLAT-3, QLT-2, QLT-7, DEP-2–6, **GOV-7** (fold audit only if it stays one endpoint after GOV-6), **SEC-8** (audit DB least-privilege role).

Sprints 29–36 are closed. Next is Sprint 37 (**ED-7**, **UI-52**). Do not start **SRV-7 (MCP)** until asked.

## Open Product Questions
- **Decided (ARCH-6 + ARCH-7, Sprints 29–33):** published constitutional versions are immutable snapshots (no in-place PATCH). Public amendment timeline = curated **laws** (and official gazette errata), not editorial version control. Snapshot chain and amendment revisions are both **linear** (no branches). Writer typos stay in an open edit session. A transcription error missed after publish is an `editorial_correction` hop on the **constitution-wide** chain tip.
- **Decided (ARCH-8 ADR 0005; Sprints 35–39 implement):** two axes — legal versions (public chips) vs editorial revisions of each law and each change record. Design: [`docs/adr/0005-two-axis-versions.md`](docs/adr/0005-two-axis-versions.md), [`docs/design/two-axis-versions.md`](docs/design/two-axis-versions.md) (**Sprint UI reference** = configured `prototype/editor-constitution.html` hashes) and [legal vs editorial chain](/Users/julianhoelz/.cursor/projects/Users-julianhoelz-Documents-Verfassung/canvases/legal-vs-editorial-chain.canvas.tsx). Assumptions: (a) hop kinds are only `initial` / `legal` / `editorial_correction` — no errata enum; (b) a change record is title + comment + documents; (c) published pins quote two snapshot ids; `needs_review` when pins ≠ live tips; quotes are not auto-rewritten; (d) the live hop always runs on current editorial tips; each snapshot has at most one change-record forward and one backward (retired pins dashed); (e) editorial publish appends to that legal version’s editorial tip; public “latest” is the editorial tip of the legal tip; (f) change-record metadata still skips the article-text reviewer queue; (g) anonymous `GET` of an editorial snapshot URL is 200 if you know the id; (h) withdraw hides a record from the public, it is not hard-deleted; (i) **Record the next legal change** is one workspace (articles + instrument sidebar); a published hop always has two constitutional versions; staff do not choose transcription vs new law in one Publish dialog.
